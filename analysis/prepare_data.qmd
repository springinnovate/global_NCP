...
message("--- Immediate post-transform validation ---")
# st_transform() can introduce small geometric errors (e.g., 2-point LinearRings)
# that cause st_point_on_surface() to fail. We must repair them immediately.
invalid_geoms_after_transform <- !sf::st_is_valid(grid_4326)
message(glue::glue("Found {sum(invalid_geoms_after_transform)} invalid geometries after reprojection. Repairing..."))
grid_4326 <- sf::st_make_valid(grid_4326)

message("4. Attaching attributes via spatial join...")
# Use point-on-surface for a robust join that avoids duplicating grid cells that touch multiple polygons
grid_pts <- st_point_on_surface(grid_4326)
...
grid_final <- grid_4326 %>%
  select(fid = ID) %>%
  left_join(grid_attrs, by = "fid")

message("5. Final validation and aggressive geometry cleaning...")
