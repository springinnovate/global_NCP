...
![Yucatan 2020 ESa Land  Cover map (reclassified) and Restoration Priority Areas](/home/jeronimo/OneDrive/WWF_nbs_op/Interventions/Mex_intervention/yucatan_LC1.png)
...
```{r select ES  data, eval=FALSE, include=FALSE}
...
# Reaorder because freaking file names #Check this, but this should be easier
...
## Obtaine baselione values
```{r  obtain baseline output, eval=FALSE, include=FALSE}

#
# Load AOI (polygon or raster)
poly <- st_read(here("Interventions", "Mexico_int_areas", "AREA_INTERVENCION2_CORREDORES.shp"))
...
griscom <- rast(here("Restoration_Griscom", "recMEXICO.tif"))
#2 Background mask (fix  this also, too complicated)
bkg_ee <- rast(here("Interventions", "Mex_intervention", "yucatan_bkg.tif"))
#Reprojections

poly <- st_transform(poly, crs=crs(griscom))
griscom <- mask(griscom, poly)
...
map(1:length(baseES), function(x) writeRaster(baseES[[x]], paste0(here('Interventions',"Mexico_int_areas"), "/", serv.[x], "_msk.tif"), overwrite=TRUE))
...
baseES_r <- align_rasters(baseES_r, bkg_proj) #Fix here, use thew original restoration layers, not repojected 
...
#load restoration mask (Griscom)
#rest_m <- rast(here('Interventions', 'Mex_intervention', 'Mex_griscom.tif'))
griscom_p <- round(project(griscom, bkg_proj, threads=20), digits=0)
griscom_p<- round(griscom_p, digits=0)
# serv_1 <- mask(serv_1,rest_m, maskvalues=0)
# Crop and mask for the new area
#rest_m <- crop(rest_m,bkg_2)
#bkg_2 <- crop(bkg_2,rest_m)

#bkg_2 <- resample(bkg_2,rest_m)
#rest_m <- mask(rest_m,bkg_2)
...
num_cores <- detectCores() - 4  # Leave 1 core free
...
results_list <- mclapply(1:num_repetitions, function(x) {
  sample_and_calculate(raster = serv_1, pixels_needed = pix_tot, confidence = 0.95)
}, mc.cores = num_cores)
...
load(here("Interventions", "Mex_intervention", "all_res_mex.RData"))
...
write.csv(final_summary2, file=here("Interventions", "Mex_intervention", "summary_interventionMex2.csv"))
```
These gains are achieved by restoring 15.000ha, or 0.35% of the intervention area.
...
plot <- ggplot(all_results, aes(y = perc_sum, fill = color)) +
  geom_boxplot() +
...
![Restoration Priority area per Service (top 10%)](/home/jeronimo/OneDrive/WWF_nbs_op/Interventions/Mex_intervention/Mex_fin.png)
