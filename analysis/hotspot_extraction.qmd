...
# Filter out Lakes and Rock & Ice from WWF_biome
if ("WWF_biome" %in% names(sf_f)) {
  sf_f <- dplyr::filter(sf_f, !WWF_biome %in% c("Lakes", "Rock & Ice"))
}
...
if (nrow(signed_keep0) > 0) {
  ...
  message("WARNING: signed_keep0 is empty. Check groupings or service mapping.")
}
...
pivot_groupings <- c("income_grp","region_wb","continent","region_un","WWF_biome","nev_name")
...
# Apply filters to plt_long (whether cached or new)
...
bar_dirs <- c(
  file.path("outputs","plots", paste0("signed_alt_keep0", params$output_suffix)),
  file.path("outputs","plots", paste0("signed_alt_drop0", params$output_suffix))
)
