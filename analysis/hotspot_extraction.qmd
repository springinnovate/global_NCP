...
if (file.exists(lc_gpkg) && exists("plt_long")) {
  ...
  # Identify the specific column for Loss of Natural (1992-2020)
  loss_col <- grep("Loss_2_.*", names(lc_df), value = TRUE)[1]
  net_col <- grep("dir_ch_2_.*", names(lc_df), value = TRUE)[1]
  if (!is.na(loss_col)) {
    message("Using LC column: ", loss_col, " as proxy for Land Conversion.")
    lc_data_ready <- TRUE
  } else {
    message("No 'Loss_2' column found in LC data.")
  }
} else {
...
overlap_stats <- NULL
if (lc_data_ready) {
  # Define LC Hotspots (Top 5% of Loss)
  cutoff <- quantile(lc_df[[loss_col]], 0.95, na.rm = TRUE)
  lc_hotspots <- lc_df %>%
    filter(.data[[loss_col]] > cutoff) %>%
    pull(fid)

  # Calculate Overlap for each Service
  overlap_stats <- plt_long %>%
    group_by(service) %>%
    summarize(
      n_es_hotspots = sum(
    ...
    mutate(pct_overlap = (n_overlap / n_es_hotspots) * 100) %>%
...
if (!is.null(overlap_stats)) {
  # Plot
  p <- ggplot(overlap_stats, aes(x = reorder(service, pct_overlap), y = pct_overlap)) +
   geom_col(fill = "#d95f02") +
   coord_flip() +
   labs(title = "Overlap with Land Conversion Hotspots",
        subtitle = paste0("% of ES Hotspots that are also in the top 5% of ", loss_col),
        x = "Service", y = "% Overlap") +
   theme_minimal()
  print(p)
} else {
