...
year <- rep(c("diff",2020,1992))
# create a table with the content to fill the table
color <- c("#68606b","#08606b", "#08606b")
...
```{r test the ch function thing}

set1 <- st_make_valid(set1)
set2 <- st_make_valid(set2)
set2_ng <- st_drop_geometry(set2)
set1 <- left_join(set1,set2_ng)
set1_ng <- st_drop_geometry(set1)

set1_ng <- set1_ng %>% mutate(HYBAS_ID = as.character(HYBAS_ID))
set3<- set3 %>% mutate(HYBAS_ID = as.character(HYBAS_ID))

set_f <- left_join(set3, set1_ng)
sf::st_write(set_f,"/Users/rodriguez/Library/CloudStorage/OneDrive-WorldWildlifeFund,Inc/global_NCP/vector/hydrosheds_lv6_synthesis.gpkg", layer = "synthesis", append = FALSE)

set_fng <- st_drop_geometry(set_f)

write.csv(set_fng, file= '/Users/rodriguez/Library/CloudStorage/OneDrive-WorldWildlifeFund,Inc/global_NCP/vector/synth_table')
```
...
#### Here, repalce this horrible thing with the fucntion. This is all hardcoded. 
poly <- poly %>%
  mutate(
    n_export_pct_ch = ((global_n_export_2020_per_ha - global_n_export_1992_per_ha) / global_n_export_1992_per_ha) * 100,
    sed_export_pct_ch = ((global_sed_export_2020_per_ha - global_sed_export_1992_per_ha) / global_sed_export_1992_per_ha) * 100
  )
