...
lc_gpkg <- file.path(data_dir(), "processed", "10k_lcc_metrics.gpkg")
if (file.exists(lc_gpkg)) {
  # Load only the relevant metrics (Loss/Gain of Natural for 1992-2020)
  # We assume the column naming convention from LC_change.qmd: Loss_2_1992_2020
  ...
  # Select columns dynamically: Loss_2 (Conversion) and Gain_2 (Restoration)
  cols_to_keep <- grep("(Loss|Gain)_2_.*", names(lc_df), value = TRUE)
  lc_df <- lc_df %>% dplyr::select(fid, dplyr::all_of(cols_to_keep))

  geom_sf <- dplyr::left_join(geom_sf, lc_df, by = "fid")
  cat("Joined Land Cover metrics to geometry.\n", file = stderr())
}
...
if (file.exists(lc_gpkg) && exists("plt_long")) {
  ...
  warning(paste0("LC data or plt_long missing. Looking for: ", lc_gpkg))
}
```
...
if (lc_data_ready) {
  # Define interesting pairs for scatterplots
  pairs <- list(
    list(driver="Urban_Exp", es="C_Risk"),
    list(driver="Urban_Exp", es="N_export"),
    list(driver="Forest_Loss", es="Nature_Access"),
    list(driver="Forest_Loss", es="Pollination"),
    list(driver="Crop_Exp", es="Sed_export"),
    list(driver="Crop_Exp", es="N_export")
  )
  ...
  for (p in pairs) {
    ...
    if (d_name %in% names(drivers)) {
      ...
      plot_data <- plt_long %>%
        ...
        filter(driver_val > 0) # Focus on areas where change occurred
      ...
      if (nrow(plot_data) > 100) {
        ...
        p_scatter <- ggplot(plot_data_sample, aes(x = driver_val, y = pct_chg)) +
          ...
          labs(title = paste(d_name, "vs", es_name, "Change"),
               x = paste(d_name, "(% Area)"),
