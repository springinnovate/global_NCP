...
Here is what we need to reframe. 
 ...
 - Add the primary key to join the metadata. Normally is the raster name (issue: raster algebra output maps get the name of one of the input products by default, and that can become a problem later, so one shouldthink on a way to add a menaingful name from here). 
...
#   10000, Inf, NA      
# ), ncol = 3, byrow = TRUE)
# rast_list1<-lapply(rast_list1, function(r){
#   r <- classify(r,rcl)
# })#, mc.cores=4)
# writeRaster(rast_list1[[1]], paste0(here('input_ES', 'mod_n_ret'), '/', "global_n_export_2020_compressed_rec.tif"))
# rast_list2<-lapply(rast_list2, function(r){
#   r <- classify(r,rcl)
# })#, mc.cores=4)
# writeRaster(rast_list2[[1]], paste0(here('input_ES', 'mod_n_ret'), '/', "global_n_retention_2020_compressed_rec.tif"))
### There is an isse here. It doesn ot work fast enough. 

#This iterates over a pair of lists. Doe not seem to be woprking correclty. Also check Sediment Again! oohh no. Sediment is correct because it was with the USLE, and the formula is differenty/ 
# serq <- mclapply(seq_along(rast_list2),function(i){
#   rast_list2[[i]]/(rast_list2[[i]]+rast_list1[[i]])}, mc.cores=2)
# 
# 
