...
The combined raster is a proxy for the total ES provided, assuming that all services are equally valuable and that the location has already been consider during the modeling.
...
The analysis was limited to tyhe to the pixels with the Global Reforestation Potential Map by Griscom et al (2017).
...
tiffes <- tiffes[1]
pa_rc <- rast(tiffes)
pa_rc <- crop(pa_rc, tmp)
 
serv_bra <- mask(serv_bra, tmp)
  
 writeRaster(serv_bra, paste0(here("restoration_combined"),'/', "BRA_ES_msk.tif"), overwrite=T)
 sf_crs <- st_crs(poly)$proj4string 
 serv_bra <- project(serv_bra, sf_crs, method= 'bilinear', res=30, threads=24)
 
 
  griscom <- rast(here("Interventions", "Brazil_intervention", "SS_griscom.tif"))
 griscom <- project(griscom, serv_bra, threads=24)
  
  serv_bra <- mask(serv_bra, griscom, maskvalue=0)
 
 target_area <- 30200  # Target area in hectares

# Step 1: Calculate pixel area in hectares
pixel_area <- res(serv_bra)[1] * res(serv_bra)[2] / 10000  # 
 r_vals <- values(serv_bra, na.rm = TRUE)  # Extract non-NA pixel values
sorted_values <- sort(r_vals, decreasing = TRUE)  # Sort in descending order

# Step 3: Accumulate pixel areas until the target area is reached
cum_area <- cumsum(rep(pixel_area, length(sorted_values)))  # Cumulative area
cutoff_index <- which(cum_area >= target_area)[1]  # Find the index for the cutoff
value_cutoff <- sorted_values[cutoff_index]  # Get the value at the cutoff index

# Step 4: Create a mask for pixels meeting the condition
msk <- serv_bra >= value_cutoff  # Retain only pixels >= cutoff value
selected_pixels <- msk * serv_bra  # Apply the mask to keep the original values
 writeRaster(selected_pixels, paste0(here("restoration_combined"),'/', "max_ES.tif"), overwrite=T)
