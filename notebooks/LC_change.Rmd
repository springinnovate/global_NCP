...
source("utils_lcc_metrics.R")
```


Here i am creating my maps of land cover change. Just from natuiral to tramnsformed. As usual, this is easier said that n done
...
landcover_files <- list.files("input_ES/landcover_reclassified", 
                              pattern = "^lcc_\\d{4}.*tif$", full.names = TRUE)
...
basins <- st_read("vector/hydrosheds_lev6.gpkg")
```

## Run Land Cover Change Metric Extraction

```{r run-extraction}
lcc_metrics_df <- iterate_zonal_lcc_metrics(
...
readr::write_csv(lcc_metrics_df, "output_data/lcc_metrics_results.csv")
```
