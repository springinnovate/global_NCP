...
# Step 2: Create the bar plot
p <- ggplot(totSE, aes(x = Service, y = Value, fill = Service)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7)) + 
  aes(width = ifelse(Service == "Area", 0.5, 0.9)) + 
  scale_y_continuous(limits = c(0, 100)) +
  facet_wrap(~ Country, ncol = 5) +  # Create 5 columns (one for each country)
  scale_fill_manual(values = c(
    "Area" = "gray50",         # Medium gray for "Share Area"
    ...
    "Pollination (People fed)" = "#dd1c77",          # Shade of purple
