...
KS_CFG <- list(
  ...
  rule_mode      = "vectors",
  # 5-service redesign (2026-08-28): fixed 2026-08-31 after this file's independent config drift
  # was found (was still using export/risk as "gain" services and ratios alone as "loss").
  #
  ...
  # ratio row came back with n_hot=0 after an earlier fix that (wrongly) restricted this to just
  # the 5 amounts.
  #
  ...
  loss_services  = c("Nature_Access","Pollination","N_retention","Sed_retention","C_Prot_service",
                      "N_Ret_Ratio","Sed_Ret_Ratio","C_Risk_Red_Ratio"),
  gain_services  = c(),
  combos = list(
...
if (need_reload) {
  ...
  canonical_lookup <- c(
    ...
    # was "rt_service" -- didn't match the real column (c_prot_service); same bug independently
    # fixed in hotspot_extraction.qmd 2026-08-31, this file has its own unsynced copy
