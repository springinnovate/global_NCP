...
#poly <- st_read(here('vector','income_group.gpkg'))
poly <- st_read('/Users/rodriguez/Library/CloudStorage/OneDrive-WorldWildlifeFund,Inc/global_NCP/vector/income_g.gpkg')
#poly <- st_read('/Users/rodriguez/Library/CloudStorage/OneDrive-WorldWildlifeFund,Inc/global_NCP/vector/biomes_WWF.gpkg')
```
...
# Initially, compute mean. I included median but it makes no sense here. Don't calculate it anymore
target_stats <- c("mean")
# this is the column in the vector file that we are going to use. 
cols <- 'BIOME'
col <- "WWF_biome" # this is for the join. Not great but gets the work done
# Number of CPU cores to use
num.cores <- 6#length(rasters_list) # set as many cores as rasters to be evaluated. I need to take a look at this, mclapply does  not seem to be improving
# Somehow using multicore seems to be problematic. It is too irregular. Sometimes works great, sometimes takes forever and i have not been able to find out  WHY. 
...
zonal_wide <- pivot_wider(zonal_df, id_cols=col, names_from=c(service, year), values_from = mean)
# adjust column names 

poly <- left_join(poly, zonal_wide, by= col)
st_write(poly, here('vector', 'biome_poly_means.gpkg'), append = FALSE) 
write.csv(zonal_df, here('output_data', 'zonal_df_biomes.csv'))

# I think this is not necessary anymore. Consider removing. 
# poly_df <- st_drop_geometry(poly) %>% select(id, ee_r264_name,iso3,continent,region_wb,income_grp,name_long)  
# poly_df <- st_drop_geometry(poly) %>% select(BIOME, WWF_biome) 
#  # if merged 
# poly_df <- st_drop_geometry(poly) #%>% select(id, ee_r264_name,iso3,continent,region_wb,income_grp,name_long)  
# zonal_df <- zonal_df %>% left_join(poly_df,by = cols)

#best to sabe as csv. RDatas somehow get corrupted all the time
```
...
zonal_wide <- pivot_wider(zonal_df, id_cols=WWF_biome, names_from=c(service), values_from = mean)
# adjust column names 
colnames(zonal_wide)[colnames(zonal_wide) != "WWF_biome"] <- paste0(colnames(zonal_wide)[colnames(zonal_wide) != "WWF_biome"], "_diff")
