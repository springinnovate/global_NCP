...
if (isTRUE(save_plot)) {
  ...
  out_dir <- file.path("outputs", "plots", metric, tolower(group_col))
...
# ---- helper: compute + plot one panel set --------------------------------
plot_change_bars <- function(df, group_col, metric = c("pct","abs"),
                             include_global = FALSE,
                             out_dir = "outputs/plots", out_stub = NULL,
...
# ---- run: ALL groupings, both metrics, w/ and w/o Global ------------------
for (gc in groupings) {
  for (m in metrics) {
    plot_change_bars(plt_long, group_col = gc, metric = m,
                     include_global = FALSE, out_dir = "outputs/plots")
    plot_change_bars(plt_long, group_col = gc, metric = m,
                     include_global = TRUE,  out_dir = "outputs/plots")
  }
}
...
# -----------------------------------------------
# Faceted violins (VERTICAL) for hotspots by group
# - Uses central HOTS_CFG (loss/gain, cutoff mode)
# - ABS and PCT plotted separately
# - Free y scales per facet; 3 columns; fixed facet order
# - Removes NA/0 and trims 0.1%–99.9% per service for plotting
# - Saves to outputs/plots/<metric>/<group_col>/violins_<group_col>_<metric>.png
# -----------------------------------------------
run_hotspot_violins_by <- function(
  ...
  plot_n = 300000L,
  keep_only_ordered = TRUE,
  out_root = file.path("outputs","plots")
) {
  stopifnot(exists("plt_long"), group_col %in% names(plt_long))
  message("==> Violins by: ", group_col)

  # 1) Extract hotspots once for ABS and once for PCT using central config
  ...
  # 2) Long value tables (fid × service × group) and align
  abs_vals <- by_abs %>%
    dplyr::transmute(
  ...
  # Helper to trim to central [0.1%, 99.9%] range per service
  ...
  # 5) Save in the same directory scheme used for bars
  # NOTE: force white background so VS Code’s dark preview is readable
