...
basins <- st_read(paste0(inpath, "/", "hydrosheds_lv6_synthesis.gpkg"))

basins <- basins[c(1:13)]

cnt <- st_read('/Users/rodriguez/Library/CloudStorage/OneDrive-WorldWildlifeFund,Inc/global_NCP/vector/cartographic_ee_ee_r264_correspondence.gpkg')
cnt <- cnt[c(1,22)]

tmp <- rast(tiffes[[1]])
#load a template
# Create a raster template that covers all basins
...
# Proceed with the same logic using 'countries_eq' and 'basins_eq'

template <- terra::rast(bas_eq, resolution = 300)  # adjust resolution as needed
...
basins$country_id <- dominant_country
basins <- left_join(basins, countries %>% st_drop_geometry() %>% select(country_id, country_name), by = "country_id")
