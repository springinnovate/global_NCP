...
#base_filename <- gsub(" ", "_", base_filename) # Replace spaces with underscores
  ...
  map(1:length(masked), function(x) {
    writeRaster(masked[[x]], paste0(outpath,"/", "ESA_LC_1992_2020_", nam[x], ".tif"), overwrite = TRUE)
  })
...
# Use lapply to iterate over each list in nested_list
  map2(rs_msk, nam, writeRaster(x., paste0(here("ESA_LC", "wb_region"),"/",  "ESA_LC_1992_2020_", y., '.tif')))
