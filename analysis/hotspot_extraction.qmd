...
force_attach <- FALSE  # set TRUE to force re-attach even if file exists
required_group_cols <- c(
  "iso3","continent","income_grp","region_un","region_wb","subregion","WWF_biome","BIOME"
)
skip_attach <- FALSE
if (file.exists(out_gpkg) && !force_attach) {
  existing <- tryCatch(sf::st_read(out_gpkg, quiet = TRUE, n_max = 1), error = function(e) NULL)
  if (is.null(existing) || !inherits(existing, "sf") || is.null(sf::st_geometry(existing))) {
    message("attach-region-attrs: existing 10k_change_calc.gpkg is non-spatial; skipping join.")
    skip_attach <- TRUE
  } else {
    missing <- setdiff(required_group_cols, names(existing))
    if (length(missing) == 0) {
      message("attach-region-attrs: existing 10k_change_calc.gpkg already has groupings; skipping join.")
      skip_attach <- TRUE
    }
  }
}
...
::: callout-note
**TODO (cleanup legacy hotspot outputs)**  
Older runs created numeric biome folders (e.g., `processed/hotspots/abs/biome/*`) which are now superseded by `WWF_biome`. After validating the new outputs, delete the legacy numeric-biome folders to avoid confusion.
:::
