...
# this omplex function adjusts names and harmonizes the whole thinng. It is a very poor way to solve this but at this point had to be done. It is easier to deal with variable names even earlier, but because reasons i did not do that.
# All this is just intended to facilitate the visualization. It was not really that fundamnetal and hopeful;ly we don't need to repeat. Or if we do, we think about this better. Anyway i hope i have trhe outputs exacly as i need them for mappinjg
...
# Need to find the helper function, do it later, to also remermber waht the point was 
...
for (i in seq_along(nams1)) { val_col \<- nams1\[i\] bin_col \<- nams2\[i\] masked_col \<- paste0(val_col, "\_masked")
...
>>>>>>> f15f93e (commit from Cumaribo, last update with Costal)
## 2.1 Hotspots
