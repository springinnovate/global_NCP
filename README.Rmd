...
Working version of a structured workflow for extracting, analyzing, and visualizing **zonal summary statistics** from global rastger datasets including **ecosystem service (ES)**, **land cover (LC)**, and socioeconomic (beneficiaries) raster datasets. The analysis focuses on detecting spatial and temporal change by syntethizing multiple modeling outputs at multiple spatial aggregation levels (e.g., countries, income groups, basins).

The core of this workflow is built around the [`exactextractr`](https://github.com/isciences/exactextractr),  availbe both for R and Python which allows for efficient zonal statistics operations between raster and vector data. More recent additions to the workflow include:

These tools and methods are designed to enable rapid and reproducible summaries of change in ES provision and land cover composition across any defined spatial units — from countries and continents to basins and sub-basins and explore relationships between these changes and the beneficiaries. 
