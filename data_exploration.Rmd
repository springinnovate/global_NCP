...
poly <- st_read(here("vector", "areas_nbs.gpkg")) 
...
```{r select pending data to process, eval=FALSE, include=FALSE}
#Remove points, risk reduction baseline
tiffes <- tiffes[-c(2,3,4)]
tiffes2 <- file.path(here(inpath), list.files(paste0(here(inpath)),pattern= 'pollination.*\\.tif$'))
tiffes2 <- tiffes2[c(2,3)]
tiffes <- c(tiffes,tiffes2)
This is pending to fix, i am just groupingthje differnet layers, butactually i could hve processed all those on the same crs together (cosnervation/restoration)
#######################
...
This runs the same process in batch. Memory was optimzied to not load all the data atthesame time, but process and write each raster individually before going to the next one.  
...
#### 4.1.1 List the conservation areas 
...
#Remove points, risk reduction baseline
tiffes <- tiffes[c(21:30,36:40,71:75)]
```
...
#Get baseline values for ecosystem services. Will use the 2020 baseline values
...
bkg <- map2(
  ...
  baseES,
  function(r1,r2){
    r1 <- resample(r1,r2[[1]], method = "near", , threads= 10)
  })
...
msk_ap <- baseES_ap[c(1,4,7,10,13)]   # there must be a smarter way to do this. It needs to work with 
