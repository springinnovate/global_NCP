...
tiffes <- file.path(inpath, list.files(paste0(inpath),pattern= 'diff'))
tiffes <- tiffes[-c(8,9)]
...
# 3. Calcualte Differences (jsut in case)
...
zonal_wide <- pivot_wider(zonal_df, id_cols=name_long, names_from=c(service,year), values_from = c(mean, median))
