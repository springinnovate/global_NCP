"""Compare the Oliveira grassland/shrubland-Kc test run (09j) against WWF-SIPA's baseline, same
pattern as `15_negronjuarez_comparison.py` (forest). Isolated against the same all-Kamble
reference point forest was tested against -- comparable apples-to-apples.
...
NCP_WORKSPACE = "data/swy/philippines/workspace_becky_inputs_90m_oliveira_grassland_kc"
NCP_SUFFIX = "oliveira_grassland_kc"
BASELINE_WORKSPACE = (
...
def run_pair(var, baseline_path, ncp_path, out_png):
    ...
    ax.set_ylabel(f"NCP CN + Oliveira grassland/shrub Kc {var} (mm/yr)")
    ax.set_title(f"Philippines {var}: WWF-SIPA baseline vs. NCP CN + Oliveira grassland/shrub Kc\n(n={n:,} pixels, r={corr:.2f})")
