...
## 4.1. Rasterize Country Data to the ESA LC
...
## 4.2. Assign Country Fid to the Grid
...
# Paths
grid_fp = "/path/to/grid_10km.gpkg"
raster_fp = "/path/to/countries_fid_300m_COMPRESSED.tif"

# Load data
grid = gpd.read_file(grid_fp)

# Function to compute stats for a subset of polygons
...
chunk_size = int(np.ceil(len(grid) / n_cores))
chunks = [grid.iloc[i:i+chunk_size] for i in range(0, len(grid), chunk_size)]
...
# Merge back with grid
grid_with_country = grid.copy()
grid_with_country["country_fid"] = stats_df["majority"].astype(int)

# Save result
grid_with_country.to_file("/path/to/grid_10km_with_country.gpkg", driver="GPKG")
print("✓ Finished with chunked parallel zonal stats")
