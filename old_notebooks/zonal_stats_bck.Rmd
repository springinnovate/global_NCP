...
inpath <- here('input_rasters', 'Data_spring')
...
# Assign services and years # IMOROVE INPUT MANAGEMENT HERE  #########
...
target_stats <- c("mean", "stdev") # We can calcualte any metric we want, but of course this means more columns and that eventually gets complicated
...
### 3.4 Extraction Data not from SPRING
...
```{r extract pot sed Retention, eval=FALSE, include=FALSE}
...
inpath <- here('input_rasters', 'final_extraction_92_2020')
...
Sure, streamline and automatize this further, but there are still too many special cases.
...
inpath <- here("input_rasters", "change_calc")
...
value_cols <- names(plt)[-1]  # assuming first column is the ID
years <- str_extract(value_cols, "\\d{4}") |> as.numeric()
years <- sort(years)
# Calculate % of change 
# Here we set the variable(s) from which we are going to extract the difference. 
...
zonal_wide <- compute_pct_change(plt, year_pairs=years, round_digits = 4)
