...
# #Optional dissolve to group by different vaiables (region, income, etc) This is not working well, weird merging of the polys. Do it in QGIS for now
# poly <- poly %>%
#   group_by(region_wb) %>%   # Group by the attribute
#   summarise(geom = st_union(geom), .groups = "drop")  # Merge geometries
...
# Yes, somehow using multicore seems to be prblematicic. 
num.cores <- 4
...
st_write(poly, '/Users/rodriguez/Library/CloudStorage/OneDrive-WorldWildlifeFund,Inc/global_NCP/vector/global_poly_means.gpkg', append = FALSE)
...
plot_ecosystem_services <- function(data, year) {
  ...
  data_prepped <- data %>%
    ...
    mutate(ee_r264_name = reorder_within(ee_r264_name, -mean, service))  

  # Step 2: Compute min/max values for ecosystem services
  ...
  range_data <- service_range %>%
    ...
    mutate(year = !!year, ee_r264_name = "dummy")

  # Step 4: Extract top 10 and bottom 10 per service
  top_10 <- data_prepped %>%
    group_by(service) %>%
  ...
  # Step 6: Reorder names to appear correctly
  filtered_data <- filtered_data %>%
    mutate(ee_r264_name = reorder_within(ee_r264_name, -mean, service))

  # Step 7: Plot the filtered data
  p <- ggplot(filtered_data, aes(x = ee_r264_name, y = mean, fill = color)) +
...
# Generate plots for different years
p_1992 <- plot_ecosystem_services(zonal_df, 1992)
p_2020 <- plot_ecosystem_services(zonal_df, 2020)
