...
- TODO (v1.4.0): Remove the temporary `st_intersects` spatial join in `process_data.qmd` used to fix the "Fragment Bug". Revert to a fast tabular join using the `orig_fid` now preserved by the Python extraction pipeline.
- TODO (Data Cleanup): Relocate root-level spatial rasters (e.g., `base_years_ha/`, `2020_1992_ch_ha/`) into the `raw/` folder to align with the directory structure, and update the corresponding YAML configuration files in `analysis_configs/` to reflect the new paths.
