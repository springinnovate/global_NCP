...
- `analysis/hotspot_extraction.qmd`: builds `plt_long` from the 10 km calc layer, extracts hotspots, and exports magnitude and signed barplots plus violins to `outputs/plots/latest/{bars|violins}` (hotspot GPKGs and index when enabled).
- `analysis/KS_tests_hotspots.qmd`: KS tests comparing hotspot vs non-hotspot distributions for socioeconomic vars; writes `outputs/tables/ks_results_hot_vs_non.csv` and plots to `outputs/plots/latest/ks`.
- `analysis/hotspot_sandbox.qmd`: lightweight playground mirroring configs; heavy chunks off by default; use for ad-hoc experiments without touching the main report.
- Upstream grid build: `summary_pipeline.py` (Python/taskgraph) uses `exactextract` to pull modeled rasters onto the 10 km grid and write `processed/10k_change_calc.gpkg` (external data dir).

## Rendering reports

- HTML: `quarto render analysis/hotspot_extraction.qmd` (same for KS/sandbox).
- PDF (TinyTeX installed): `quarto render analysis/hotspot_extraction.qmd --to pdf`. If missing TinyTeX, install once via `quarto install tool tinytex` or print the HTML to PDF in a browser.
- Chunk caches speed reruns; keep `_cache/` intact unless you need a clean recompute.
...
- Replace heavy long pivot with per-service aggregation; make KS stratified runs (e.g., by income group/region) a first-class option
