...
This workflow outlines a structured approach to extract and analyze zonal statistics for **Ecosystem Services (ES)** at the global scale using **hydrological basins** as spatial units, based on the HydroBASINS Level 6 and 7  from [HydroSHEDS](https://www.hydrosheds.org/products/hydrobasins).

Buoding upon the broader multi-service analysis conducted for 1992 and 2020, this pilot focuses exclusively on the **Nutrient Delivery Ratio (NDR)** model, assessing changes in nitrogen export and retention for five time points: **1992, 1995, 1998, 2001, and 2004**. The purpose is to test the workflow over time, extract consistent zonal statistics, and begin exploring temporal dynamics in nutrient regulation.

This is a **preliminary version**, developed while we address technical issues with the global geosharding implementation, which currently cannot process locations located **above 60° North**. Despite that, this workflow allows us to:
- Validate the core zonal processing steps,
- Generate preliminary results for exploratory analysis, and
- Ensure the overall reproducibility and readiness of the methodology once full global coverage becomes available.

# 1. Objectives

1. Extract zonal summary statistics for modeled **NDR Export and Retention** layers at five time points using the `exactextractr` package.
...
> **Note:** The hydrological basin appears to be a promising spatial unit for ecosystem service analysis, providing consistency across scales and enabling meaningful aggregation for regional and global assessments.

A new iteration includes hexes/gridded area.

---

Let me know if you’d like this added to an `.Rmd` file or need help formatting the rest of the workflow!
...
#st_write(poly, (here('vector', 'hydrosheds_lv7.gpkg')), append=FALSE)
# This is to add a temporal attribute to the data. I am still experimenting here, trying to get some animations or more refined visualizations 
...
- Extract Spatial units with the highest increases/decreases date to date in the variables of interest
- Produce Maps (working on it)
- Create glossary with the variable/column names. 
- Make suree naming conventions are consistent. 
- Knit /finish explainer
