...
message("2. Filtering grid to land areas by intersection...")
grid_land <- st_filter(grid_raw, st_transform(land_polys, st_crs(grid_raw)), .predicate = st_intersects) %>%
  mutate(ID = row_number()) # Create a new, stable ID after filtering to land.

message("3. Reprojecting to EPSG:4326...")
grid_4326 <- st_transform(grid_land, "EPSG:4326")

message("4. Fixing dateline wraparound artifact...")
grid_clean <- st_wrap_dateline(grid_4326, options = c("WRAPDATELINE=YES", "DATELINEOFFSET=180"))

message("--- Immediate post-wrap validation to fix invalid LinearRings ---")
# st_wrap_dateline can produce invalid geometries (e.g., 2-point LinearRings).
# st_point_on_surface will fail on these. We must repair them immediately.
invalid_pre_join <- !sf::st_is_valid(grid_clean)
message(glue::glue("Found {sum(invalid_pre_join)} invalid geometries before spatial join. Repairing..."))
grid_clean <- sf::st_make_valid(grid_clean)

message("5. Attaching attributes via spatial join (post-dateline wrap)...")
# Use point-on-surface for a robust join that avoids duplicating grid cells that touch multiple polygons
grid_pts <- st_point_on_surface(grid_clean)

grid_attrs <- grid_pts %>%
  st_join(st_transform(land_polys, st_crs(grid_pts)) %>% select(any_of(c("iso3", "nev_name", "country", "region_wb", "income_grp", "continent", "region_un", "subregion")))) %>%
  st_join(st_transform(biome_sf, st_crs(grid_pts)) %>% select(WWF_biome)) %>%
  st_drop_geometry() %>%
  # Conditionally rename 'country' to 'nev_name' if 'nev_name' doesn't already exist.
  {
    df <- .
    if (!"nev_name" %in% names(df) && "country" %in% names(df)) {
      df <- dplyr::rename(df, nev_name = country)
    }
    df
  } %>%
  select(fid = ID, any_of(c("iso3", "nev_name", "region_wb", "income_grp", "continent", "region_un", "subregion", "WWF_biome"))) %>%
...
message("6. Final validation and aggressive geometry cleaning...")
invalid_geoms <- !sf::st_is_valid(grid_final)
if (any(invalid_geoms)) {
  message(glue::glue("Found {sum(invalid_geoms)} invalid geometries. Attempting to repair..."))
  grid_final <- sf::st_make_valid(grid_final)

  # Add a parallelized zero-buffer to rebuild geometries, which can fix more stubborn issues
  message("Applying parallelized zero-buffer to fix complex geometry errors...")
  n_cores <- parallel::detectCores() - 2
  if (is.na(n_cores) || n_cores < 1) n_cores <- 1

  message(glue::glue("Splitting data into {n_cores} chunks for parallel processing..."))
  row_indices <- 1:nrow(grid_final)
  chunks <- split(row_indices, cut(row_indices, n_cores, labels = FALSE))
  grid_chunks <- lapply(chunks, function(idx) grid_final[idx, ])

  processed_chunks <- parallel::mclapply(grid_chunks, function(chunk) {
    sf::st_buffer(chunk, dist = 0)
  }, mc.cores = n_cores)

  message("Recombining parallel chunks...")
  grid_final <- do.call(rbind, processed_chunks)
}
empty_geoms <- sf::st_is_empty(grid_final)
if (any(empty_geoms)) {
    message(glue::glue("Found and removed {sum(empty_geoms)} empty geometries."))
    grid_final <- grid_final[!empty_geoms, ]
}
...
st_write(grid_final, out_path, delete_dsn = TRUE)
