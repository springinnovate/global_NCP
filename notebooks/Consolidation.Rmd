...
# create vector with the service names
services <- unique(plt_long$service)
# STEP 1: Identify Top/Bottom % Hotspots Per Service
#This function sorts and labels the hotpots by individual services in ascending and descending order based on the provided cutoff value
# It adds following columns: total rows per service, cutoff value, rank (ascending and descending) wether it is a hotspot or not and a binary column with the same information
...
# this stpe processes df_hotspots to extract thte fid of the units that are a hotspot, labels the variables in terms of ecosystem service loss/increased damage and adds a colunmn that list the variables for which the unit is a hotspot. 
...
# This step adds a definitopn to the hotspot type (ES loss/increased damage and specifies which one(s)) focuisng only on the damage/losses
...
# STEP 4. Split the hotspot services field into rows (so i can build the new columns)
hotspot_long <- hotspot_summary %>%
  select(fid, hotspot_services) %>%
  separate_rows(hotspot_services, sep = ",\\s*")  # split by comma and optional space


# STEP 5. Pivot wider to have a binary matrix with the services
hotspot_wide <- hotspot_long %>%
  mutate(is_hotspot = 1) %>%
  pivot_wider(names_from = hotspot_services, values_from = is_hotspot, values_fill = 0)


hotspots_f <- left_join(sf_f, hotspot_wide)
# Export aoutputs (this also needs to be adjusted, it makes no sense to kee storing this inb two differnet places)

st_write(hotspots_f, paste0(inpath, '/',"10k_grid_hotspots.gpkg"), append=FALSE)
