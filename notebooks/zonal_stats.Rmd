...
Do this by country. Running again Using the fids for the polygons. Using country names is problematic because of geopolitics.
Pending to do: a join to reasign the country names (we will end with the names of all territories, else this does not work. The join needs to add names, dependnecies among others). Think about the best way to this: directly to the vector file or just extract the target attributes, add them to the table and then somehow join back?
...
Here, i still need to think how to deal wit a single one (diff) instead of pairs. Sounds banal, but keep in mind that it's me who we are talking about and nothing ever goes smoothly for me. Becasue of course. It's me. 

# Pending: get the single year and re run the process. But not today 
```{r get differences}
...
map(1:length(diffs), function(x) writeRaster(diffs[[x]], paste0(outpath, "/", lab[x], '_diff.tif')))
```
...
results_list <- mclapply(diffs, function(r) {
  # Perform exact extraction
  # - append_cols = "country" retains the country identifier with each result
  res <- exact_extract(r, poly_wb,
                       fun = target_stats,
                       append_cols = "region_wb")
  r_name <- names(r)
