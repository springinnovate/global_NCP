...
# freq_ap <- rbind(freq_ap,freq_ap,freq_ap)
# freq_ap <- freq_ap %>% arrange(country)
freq_ap <- freq_ap %>% select(!c(1,2))
# freq_all <- rbind(freq_all,freq_all,freq_all)
# freq_all <- freq_all %>% arrange(country)
# freq_all <- freq_all %>% select(!c(1,2))
...
baseES[[1]][[2]] <- resample(baseES[[1]][[2]], baseES[[1]][[1]], method= 'cubic', threads=10)
baseES[[2]][[2]] <- resample(baseES[[2]][[2]], baseES[[2]][[1]], method= 'cubic', threads=10)
baseES[[3]][[2]] <- resample(baseES[[3]][[2]], baseES[[3]][[1]], method= 'cubic', threads=10)
baseES[[4]][[2]] <- resample(baseES[[4]][[2]], baseES[[4]][[1]], method= 'cubic', threads=10)
baseES[[5]][[2]] <- resample(baseES[[5]][[2]], baseES[[5]][[1]], method= 'cubic', threads=10)
 ...
 baseES_ap <- map2(
  ...
  function(rasters, msk) {
    # Apply mask to each raster in the sublist using the corresponding country vector
    lapply(rasters, function(raster) mask(raster, msk, maskvalues=1))
  }
)
# "api" stands for "outside". I don´t know why I gave tho name, it makes no sense 
 baseES_api <- map2(
  ...
  function(rasters, msk) {
    # Apply mask to each raster in the sublist using the corresponding country vector
    lapply(rasters, function(raster) mask(raster, msk, maskvalues=0))
  }
)
