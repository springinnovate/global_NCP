"""Paddy-rice Kc test: 09l (our CN, our per-pixel Kc) with Annual Crop's Kc replaced by the
MapSPAM-weighted FAO-56 rice curve on the Sacks calendar (07h_build_kc_paddy_rice.py). Everything
else identical to 09l, so 09l -> 09n isolates the paddy Kc change. Compare against the 09h
reference (her CN + Kc, 90m).
    ...
    MSYS_NO_PATHCONV=1 docker run --rm -w /scripts_root -e SWY_DATA_ROOT=/scripts_root/data \\
        ...
        /scripts_root/Python_scripts/swy_philippines_run/09n_run_swy_ph_paddy_kc.py
"""
import os
...
def main():
    ...
    workspace = os.path.join(DATA_ROOT, "swy/philippines/workspace_becky_inputs_90m_paddy_kc")
    ...
    kc_dir = os.path.join(DATA_ROOT, "swy/philippines/inputs/kc_paddy_2020")
    kc_path_args = {
