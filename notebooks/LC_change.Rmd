...
landcover_files <- landcover_files[c(1,9)]
...
basins <- st_read(here("vector", "countries_lc2.gpkg"), layer= "1992_2020_lcc_change_metrics_3") 
```
...
##################################################
# Run the function with only two dates # Multicore is giving us trouble for some reason
lcc_m2 <- extract_lcc_metrics(landcover_rasters[[1]], landcover_rasters[[2]], basins, col, percent=TRUE, mc = FALSE, ncores = 6, digits = 3)
# get quantity in terms of gain  - loss (the function does not provide the direction of the change, only the absolute value. 
lcc_m2 <- lcc_m2 %>%
  filter(!grepl("^Error", Category))
...
lcc_metrics_wide <- lcc_m %>%
  tidyr::pivot_wider(
    ...
    names_glue = "{.value}_{Category}" #_{year_step}" # this is commented because there are only two periods, but it can iterate over multiple time steps. 
  )
...
# Convert primary kay in both dataframes to character, just to be safe
...
sf::st_write(basins, here("vector", "countries_lc2.gpkg"), layer = "1992_2020_lcc_change_metrics_3", append = FALSE)
