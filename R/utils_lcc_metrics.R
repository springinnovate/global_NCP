...
extract_zonal_lcc_metrics <- function(r1, r2, polygons, id_col = "HYBAS_ID", 
                                      year_step = NULL, percent = TRUE) {
  stopifnot(inherits(r1, "SpatRaster"), inherits(r2, "SpatRaster"), inherits(polygons, "sf"))
  stopifnot(nlyr(r1) == 1, nlyr(r2) == 1)
  
  all_results <- lapply(1:nrow(polygons), function(i) {
    poly_i <- polygons[i, ]
    fid <- poly_i[[id_col]]
    
    # Crop/mask rasters to polygon
    ...
    # Compute contingency table and metrics
