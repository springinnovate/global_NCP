...
plt_long <- plt %>%
  ...
  mutate(service = str_replace(service, "_abs_chg.*", "")) %>% 
...
```{r load country gpkg}
ct <- st_read("/home/jeronimo/data/global_ncp/vectors/cartographic_ee_ee_r264_correspondence.gpkg")
# spliot by: $ continent, income_grp, 
ct <- ct %>% select(id, ee_r264_name, iso3, continent, income_grp, region_un,region_wb, subregion)


ct <- st_drop_geometry(ct)

biomes <- st_read("/home/jeronimo/data/global_ncp/vectors/Biome.gpkg")
biomes <- biomes[c(1,2)]




sf_template <- sf_f %>% select(fid, c_fid)
...
pts_ctdf <- st_drop_geometry(pts_ct)
pts_bmdf <- st_drop_geometry(pts_biome)
...
pts_f <- left_join(pts, out)

pts_f <-st_drop_geometry(pts_f)

sf_f <- left_join(sf_f, pts_f, by= "fid")

st_write(sf_f, "~/data/global_ncp/processed/10k_change_calc.gpkg", append = FALSE)
