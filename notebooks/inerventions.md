...
-   **Raster Processing Tools:** `terra`, `dplyr`, and `sf` packages in R.
...
```{r load intevention data, include=FALSE}
...
We run a stratified random sampling to estimate the mean values of different bands in a multi-band raster dataset. Stratified random sampling is a probability sampling method
where the population is divided into homogeneous
strata, and random samples are taken from each one.
