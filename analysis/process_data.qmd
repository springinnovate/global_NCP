...
if (length(gpkg_files) < 2) {
  stop("Could not find both services and beneficiaries GPKGs in the workspace!")
}

message("Loading Run 1 (Most Recent): ", basename(gpkg_files[1]))
run1 <- sf::st_read(gpkg_files[1], quiet = TRUE)
if (!"fid" %in% names(run1)) run1$fid <- seq_len(nrow(run1))

message("Loading Run 2: ", basename(gpkg_files[2]))
  run2 <- sf::st_read(gpkg_files[2], quiet = TRUE)
if (!"fid" %in% names(run2)) run2$fid <- seq_len(nrow(run2))

# Combine them. Both have the exact same 'fid' foundation from build_master_grid.py!
cols_to_join <- c("fid", setdiff(names(run2), names(run1)))
  sf_f <- run1 %>% dplyr::left_join(sf::st_drop_geometry(run2)[cols_to_join], by = "fid")
