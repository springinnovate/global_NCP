...
lab <- list.files(wd, pattern="tif")
lab <- lab[c(11:15,21:25,41:45)]
lab <- sapply(lab, function(x) {
  basename(tools::file_path_sans_ext(x))
})


tiffes <- file.path(wd, list.files(wd, pattern="tif"))
tiffes <- tiffes[c(11:15,21:25,41:45)]
#
#This will be done for access for 2020 to add
tiffes <- tiffes[c(66:70)] 
...
# This was to fix the order of the lists of ecosystem services ws the list of protected areas. I don;'t remeber why this happened and why i decided this was a good way to  fixing it. 
s1 <- list(baseES[[1]],baseES[[6]],baseES[[11]])
s2 <- list(baseES[[2]],baseES[[7]],baseES[[12]])
s3 <- list(baseES[[3]],baseES[[8]],baseES[[13]])
s4 <- list(baseES[[4]],baseES[[9]],baseES[[14]])
s5 <- list(baseES[[5]],baseES[[10]],baseES[[15]])


st <- map2(baseES, ap, mask)
...
baseES_ap <- list(s1[[1]],s2[[1]],s3[[1]],s4[[1]],s5[[1]],s1[[2]],s2[[2]],s3[[2]],s4[[2]],s5[[2]],s1[[3]],s2[[3]],s3[[3]],s4[[3]],s5[[3]])
...
# msk_all <- mclapply(baseES, function(r){
#   r <- subst(r, from = c(1: 1000000), to =1)
...
percentile_98 <- baseES_vf %>%
  ...
  summarize(threshold_98 = quantile(Value, 0.98)) %>%
  ungroup()
percentile_95 <- baseES_vf %>%
  group_by(Service, Country) %>%
  summarize(threshold_95 = quantile(Value, 0.95)) %>%
  ungroup()
...
ggplot(baseES_vf_filtered, aes(x = Value, fill = set)) +
  geom_density(alpha = 0.4) +  # Use transparency with alpha to see overlapping densities
  facet_grid(Service ~ Country, scales = "free") +  # Set both x and y axes to be free for each facet
  scale_fill_manual(values = c("blue", "orange")) +  # Set colors for the `set` variable
  labs(title = "Density Plot of Services by Country and Set",
       x = "Value",
       y = "Density",
       fill = "Set") +  # Label for the fill legend
  theme_minimal() +
  theme(
...
Country <- c("BRAZIL", "MADAGASCAR", "MEXICO", "PERU","VIETNAM")

C <- 5

t2 <- baseES_vf_filtered %>% filter(Country=="VIETNAM")

t# Create the faceted density plot
ggplot(t2, aes(x = Value, fill = set)) +
  geom_density(alpha = 0.4) +  # Use transparency with alpha to see overlapping densities
  facet_wrap(~Service, scales = "free") +  # Set both x and y axes to be free for each facet
  scale_fill_manual(values = c("blue", "orange")) +  # Set colors for the `set` variable
  labs(title = paste("Density Plot of Services Inside and Outside Protected Areas -", Country[C]),
       x = "Value",
       y = "Density",
       fill = "Set") +  # Label for the fill legend
  theme_minimal() +
...
ggplot(totSE_l, aes(x = Metric, y = Value, fill = Service)) +
...
scale_offset <- terra::sources(rest[[1]])
print(scale_offset)
categories(rest[[1]])
hist(values(rest[[1]]), breaks = 100, main = "Distribution of Raster Values", xlab = "Value")
