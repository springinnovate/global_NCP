...
ct <- st_read('/Users/rodriguez/Library/CloudStorage/OneDrive-WorldWildlifeFund,Inc/global_ES_TimeSeries/vector/cartographic_ee_nev_name_correspondence.gpkg')
...
df <- df %>%
  mutate(nev_name = reorder(nev_name, -dir_ch_2))
df. <- df %>% filter(service != "GDP") %>% filter(service != "Pop_sqkm")
# 4. Create plot
p <- ggplot(df., aes(x = nev_name, y = pct_ch_trans, fill = nev_name)) +
  geom_violin(trim = FALSE, scale = "width") +
  facet_wrap(~ service, scales = "free_y") +
...
# 4. Create plot
p2 <- ggplot(df, aes(x = nev_name, y = pct_ch, fill = nev_name)) +
  geom_violin(trim = FALSE, scale = "width") +
  facet_wrap(~ service, scales = "free_y") +
  scale_fill_viridis_d(option = "D") +
  labs(
...
df_top_bottom <- df_top_bottom %>%
  mutate(nev_name = fct_reorder(nev_name, -dir_ch_2))
df_top_bottom <- df_top_bottom %>% filter(service != "GDP") %>% filter(service != "Pop_sqkm")

# 3. Create plot (pseudo-log scale)
p <- ggplot(df_top_bottom, aes(x = nev_name, y = pct_ch_trans, fill = service)) +
  geom_violin(trim = FALSE, scale = "width") +
  facet_wrap(~ service, scales = "free_y") +
  scale_fill_viridis_d(option = "D") +
  labs(
...
p2 <- ggplot(df_top_bottom, aes(x = nev_name, y = pct_ch, fill = service)) +
  geom_violin(trim = FALSE, scale = "width") +
  facet_wrap(~ service, scales = "free_y") +
  scale_fill_viridis_d(option = "D") +
  labs(
