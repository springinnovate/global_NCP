"""Compare the combined forest+grassland/shrub Kc test run (09k) against WWF-SIPA's
baseline, same pattern as 15_negronjuarez_comparison.py and
17_oliveira_comparison.py. The real question: do the two independently-validated
effects (Closed/Open Forest via Negron Juarez, Brush-Shrubs/Grassland via Oliveira)
simply stack when combined, or interact -- and how much of the AOI-wide B gap they
close together now that both land at once (still leaving cropland, 43.7% of the
AOI and the single worst B distortion at 5.82x, untouched).

QF is included for a sanity check only -- Kc doesn't feed QF, so it should be
~identical to 09e_run_swy_ph_snapped_grid.py's own QF (same CN, same DEM/grid,
only Kc changed).
"""
import os
...
NCP_WORKSPACE = "data/swy/philippines/workspace_becky_inputs_90m_combined_forest_grassland_kc"
NCP_SUFFIX = "combined_forest_grassland_kc"
BASELINE_WORKSPACE = (
...
def run_pair(var, baseline_path, ncp_path, out_png):
    ...
    ax.set_ylabel(f"NCP CN + combined forest/grassland Kc {var} (mm/yr)")
    ax.set_title(f"Philippines {var}: WWF-SIPA baseline vs. NCP CN + combined forest/grassland Kc\n(n={n:,} pixels, r={corr:.2f})")
