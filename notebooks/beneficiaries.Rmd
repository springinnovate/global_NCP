...
for (metric in lc_metrics) {
  ...
  p2 <- df %>%select(c())
    ...
    scale_fill_viridis_c(
        option = "D", 
        direction = 1,
        oob = scales::squish,  # Squishes out-of-bounds values to min/max
        limits = quantile(df$pct_ch, c(0.02, 0.98), na.rm = TRUE),  # Or use `..count..` if needed
        name = "Density"
      ) +
...
p2 <- ggplot(df %>% filter(service== "Sed_ret_Ratio"), aes_string(x = "pct_ch", y = "farmsize_mehrabi.1_mean")) +
    geom_hex(bins = 60) +
    scale_fill_viridis_c(
        option = "D", 
        direction = 1,
