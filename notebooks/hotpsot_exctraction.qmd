...
st_write(hotspots_sf, here("output_data", "hotspots_5_final.gpkg"), append=FALSE)

sf_f_2 <- sf_f %>% filter(fid %in% hotspots_sf$fid)
...
hp2 <- res$summary_sf
hp2 <- hp2 %>% filter(fid %in% hotspots_sf$fid)
hp2$c_fid.y <- NULL 
hp2$hotspot_services <- NULL # i think this is not necessary to begin with )

hp <- coalesced_left_join(hotspots_sf,hp2, by= "fid")


hp$fid <- NULL #Remove fid because it won't write
st_write(hp, here("output_data", "hotspots_5_final.gpkg"), append=FALSE)
