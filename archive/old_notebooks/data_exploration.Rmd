...
sapply(packs, install.packages, character.only = TRUE, quietly=TRUE)
sapply(packs, require, character.only = TRUE, quietly=TRUE)
rm(packs)   
```

## 1.1 Set simplified Land Cover Legend

I might not need this anymore in the future. 
...
#poly <- st_transform(poly, crs = crs(acces_rest))
#Split the vector into a list of individual polygons
#create vector with names
poly <- poly%>%split(.$COUNTR)
```

## 2.1. Serv_rasters data
I think for Roadmap 2030 i really simplified all this stuff here  (July 2025)

```{r select input rasters data to process}

inpath <- here(("Downloaded_data_ES2"))

#This is one of the most basic but also annoying parts of this workflow; select the rasters to process. I have not been able to come with an autoamted manner, in some cases the strings in the product names have no common patterns that i can use, the easiest way is just to sue excel to sort and filter. Just by hand is difficult and tiresome. 
# July 2025. I think i took care of this in a way more effective way. 
#Restoration:
...
tiffes <- tiffes[c(3,23,25,26,30)] # this is stupid. I don;t remember exactly how it went at the endm but not like thi!!!
tiffes <- tiffes[-5]
print(tiffes)
...
# I think we don't need this anymore. We manage the whole thing at once, instead of dealing with reprojecting rasters. Need to remember where i have that to rebuild. 
...
This runs the same process in batch. Memory was optimized to not load all the data at the same time, but process and write each raster individually before going to the next one.  I don't even know if this is still necessary....
...
distance raster and an equal distance projection is required. I think we don't need this anymore.
...
freq_ap <- lapply(pa_rc, function(r){
  f <- freq(r)
})
country_names <- unique(country_names)
...
#grt templates
tmp <- lapply(baseES, function(sublist) sublist[[1]])


# Function to align a nested list of rasters with corresponding templates
align_list <- function(raster_list, template, resample_method = "near") {
...
baseES <- lapply(seq_along(baseES), function(i) {
  align_list(
    raster_list = baseES[[i]],
    template = pa_rc[[i]],
...
pa_rc <- mapply(function(raster, target) {
  # Reproject to match the CRS of the target raster
  reprojected <- project(raster, target, method = "near")
  
  # Resample to match the resolution of the target raster
  resampled <- resample(reprojected, target, method = "near")
  
  return(resampled)
}, pa_rc, tmp, SIMPLIFY = FALSE)
rm(tmp)
# put the accesibility data in a different list because crs

# Step 2: Apply mask for each group of rasters using the corresponding country vector
baseES_ap <- map2(
  baseES,   # The list of raster sublists
  pa_rc,     # The list of country polygons
  function(rasters, vector) {
    # Apply mask to each raster in the sublist using the corresponding country vector
    lapply(rasters, function(raster) mask(raster, vector, maskvalue=1))
  }
)
# "api" stands for "outside". I don´t know why I gave tho name, it makes no sense 
baseES_api <- map2(
  ...
  function(rasters, vector) {
    # Apply mask to each raster in the sublist using the corresponding country vector
    lapply(rasters, function(raster) mask(raster, vector, maskvalue=1, inverse=TRUE))
  }
)
...
p <- ggplot(totSE, aes(x = Metric, y = Value, fill = Service)) +
  ...
  scale_fill_manual(values = c(
    ...
    "Pollination (People fed)" = "#dd1b56",          # Shade of purple
