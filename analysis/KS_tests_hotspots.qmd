...
::: callout-note
**Exporting KS results for peer review and further interpretation**
...
- **For Python users:**
  ...
  ```
- **For other tools:** CSV is a universal format and can be imported into Excel, Stata, etc.

If you need a different format (e.g., Feather, Parquet), you can easily convert it in R or Python.
:::
## Setup
...
if (file.exists(rds_path)) {
  ...
  if (all(KS_CFG$vars_to_test %in% names(plt_long))) need_reload <- FALSE
}
...
p_direction <- ggplot(ks_res_plot, aes(x = service, y = cliffs_delta, fill = cliffs_delta)) +
  ...
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0, name = "Cliff's Delta") +
