...
We present a preliminary assessment approach to estimate potential ecosystem service gains derived from 
...
## 1.4 Next Steps
- Incorporate  benefited population (Beneficiaries: Number of people downstream of every pixel) 
- Incorporate more ES into the analysis starting with: 
...
## 3.2 Evaluated Ecosystem Services

In this analysis, the input data consists of spatial grids (raster data in GeoTIFF format), where each pixel’s value reflects the estimated gain—measured in units of the specified ecosystem service—if that area were restored to a natural or seminatural state.
...
### 3.3.2 Prepare Computing Environment
...
process_rasters <- function(nested_list, templates, resample_method = "bilinear") {
  lapply(seq_along(nested_list), function(i) {
    ...
    aligned <- lapply(ras_list, function(r) {
      if (!compareGeom(r, templates, stopOnError = FALSE)) {
        message("Aligning raster: ", names(r))
        resample(r, templates, method = resample_method)
      } else {
        message("Raster already aligned: ", names(r))
        r
      }
...
To perform the analysis, all the raster datasets need to be brought to the same spatial resolution, crs and aligned  to the same origin.
...
## 4.2 Normalize and combine ES provision gains


The combined raster is a proxy for the total ES provided, assuming that all services are equally valuable and that location has already be considering during the modeling.
It does not include Nature Access, as the pixel values repesent the amount of people that gain access to each individual pixel, which would result in counting the same people multiple times 
...
baseES <- lapply(baseES, function(r){
  ...
  aligned <- lapply(baseES, function(r) {
    ...
    merged <- merge(stacked, tmp)
    
    # Sum the layers of the merged raster
    baseES <- app(merged, sum)
normalize_raster <- function(r) {
  min_val <- min(values(r), na.rm = TRUE)
  max_val <- max(values(r), na.rm = TRUE)
  (r - min_val) / (max_val - min_val)
...
map(1:length(baseES), function(x) writeRaster(baseES[[x]], paste0(here("restoration_combined"),'/', unique(country_names)[[x]], 'BRA_ES_sum.tif')))
```

## 4.3 Extract target pixels using Global Restoration Priority Areas

The analysis was limited to tyhe to the pixels with  Restoration Potential as per Griscom et al (2017) [Global
...
random sampling. This second part has implicit following assumptions:

-   Spatial configuration effecs are already considerd when modeling theecosystem servie data 
-   The model can be refined by incorporating additional
    parameters (e.g minimum distance to boundaries,distance between 
    points, topography or inhabited areas, i .
...
used to build the final polygon: the jaguar corridor and 

### 4.2.1
Prepare Data Yucatan. Align and resample all the raster bands to the same
crs and origin.
