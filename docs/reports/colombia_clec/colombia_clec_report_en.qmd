...
:::
::: {.finding}
**Opportunity: road infrastructure.** Sediment export — one of the two services with the highest concentration of change hotspots in Colombia — is directly tied to erosion and runoff induced by road construction. The opportunity: use this same layer to assess, before construction, the erosion/sedimentation risk of proposed road corridors — not just observe the pattern after the fact. This fits into the earliest stage of the Green Road Infrastructure Guidelines (Lineamientos de Infraestructura Verde Vial, LIVV): a standardized, repeatable risk signal for corridor selection.
:::
:::
...
::: {.map-card}
![Cells identified as change hotspots (top 5th percentile of most severe change) in 1 or more of the 5 services evaluated. The strongest concentration is in three departments, nearly tied and well above the rest: Meta, Caquetá, and Antioquia.](colombia_hotspot_map.png)
:::
...
::: {.map-card}
![Four classes: priority cells, critical-asset only, change-hotspot only, neither. The strongest "Priority" concentration is in Meta, Caquetá, and Antioquia — the same departments already visible on the hotspot map.](colombia_priority_overlap_map.png)
:::
