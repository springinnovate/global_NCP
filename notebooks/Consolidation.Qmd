...
```{r add c protection, eval=FALSE, include=FALSE}
# Update: include the coastal protection data:
sf_cp <- st_read('/Users/rodriguez/Library/CloudStorage/OneDrive-WorldWildlifeFund,Inc/global_NCP/summary_pipeline_workspace/grid_10k_synth_zonal_2025_07_20_23_45_27.gpkg')
...
## 1.2 Include Coastal Protection (all Columns) 
This apprach was not great, because I did the calculation on QGIS 
...
cp2_joined <- st_join(cp2, cp_points) %>%
  group_by(fid) %>% # use the fid or any other unique identifier fro mthe polygon dataset.
  summarise(
...
plt_long <- plt_long%>% filter(service != "coastal_protection_Rt") #why??
