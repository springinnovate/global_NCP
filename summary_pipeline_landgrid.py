...
def load_config(config_path, data_root=None):
    ...
    If ``data_root`` is provided (or configured in the YAML), legacy paths
    starting with ``data/`` or ``../data/`` are rewritten relative to that
    root. Environment variables (e.g., ``${GLOBAL_NCP_DATA}``) are expanded.
    ...
    if cfg_data_root:
        cfg_data_root = Path(os.path.expandvars(cfg_data_root)).expanduser().resolve()
        data_root = cfg_data_root
    if data_root:
        os.environ.setdefault("GLOBAL_NCP_DATA", str(data_root))
...
def main():
    ...
    parser.add_argument(
        "--data-root",
        type=Path,
        default=os.getenv("GLOBAL_NCP_DATA", ""),
        help=(
            "Optional data root to resolve legacy '../data/..' paths or "
            "use in ${GLOBAL_NCP_DATA} expansions."
        ),
    )
    args = parser.parse_args()
    data_root = args.data_root if str(args.data_root) else None
    pipeline_config = load_config(args.config_yaml_path, data_root=data_root)
