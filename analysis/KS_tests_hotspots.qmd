---
title: "Relative Socioeconomic Shift"
subtitle: "v1.3.4"
...
Crucially, for the KS test, this distinction has **no impact on the results**.
*   The KS test is non-parametric and rank-based; it is invariant to monotonic transformations.
*   Because we use an equal-area grid, the `sum` and `mean` for any given variable are perfectly proportional (`sum = mean * constant_area`).
*   Therefore, the D-statistic and p-value will be identical whether the test is run on the `_sum` or `_mean` version of a variable. The choice only affects the interpretation of the descriptive statistics (e.g., `mean_hot`, `median_hot`).
...
| `p_perm` | Optional permutation p-value (from label shuffling). `NA` if permutations were not run. |
| `n_hot` | Number of grid cells in the hotspot group. |
| `n_non` | Number of grid cells in the non-hotspot (comparison) group. |
...
| `q10_hot`, `q90_hot` | 10th and 90th percentiles of the variable in the hotspot group. |
| `q10_non`, `q90_non` | 10th and 90th percentiles of the variable in the non-hotspot group. |
| `median_delta` | `median_hot − median_non`. The direction of the shift in the median. |
| `mean_delta` | `mean_hot − mean_non`. The direction of the shift in the mean. |
| `auc` | Area Under the Curve (AUC) of the ROC curve. Represents the probability that a randomly chosen observation from the hotspot group is greater than one from the non-hotspot group. A value of 0.5 indicates identical distributions. |
| `cliffs_delta` | A non-parametric effect size (`2*auc - 1`) that quantifies the magnitude and direction of the difference. Ranges from -1 (all hotspot values are smaller) to +1 (all hotspot values are larger). A value of 0 means the distributions are perfectly overlapping. Rule-of-thumb for magnitude: `|d| < 0.147` (negligible), `|d| < 0.33` (small), `|d| < 0.474` (medium), `|d| ≥ 0.474` (large). |
| `p_hot_greater` | One-sided KS p-value testing if the hotspot distribution is stochastically greater than the non-hotspot one. |
| `p_hot_less` | One-sided KS p-value testing if the hotspot distribution is stochastically smaller than the non-hotspot one. |
| `p_adj` | Benjamini–Hochberg FDR-adjusted version of `p_value` (corrected for multiple comparisons). |
| `p_hot_greater_adj` | FDR-adjusted version of `p_hot_greater`. |
| `p_hot_less_adj` | FDR-adjusted version of `p_hot_less`. |
...
- **For R users:**
  ```r
  ks_res <- read.csv(file.path("data", "processed", "tables", "ks_results_hot_vs_non.csv"))
  ```
- **For Python users:**
  ```python
  import pandas as pd
  ...
  ks_res = pd.read_csv(f'{processed_data_dir}/tables/ks_results_hot_vs_non.csv')
  ```
:::
## Setup
...
if (file.exists(rds_path)) {
  ...
  if (all(KS_CFG$vars_to_test %in% names(plt_long)) && "N_retention" %in% unique(plt_long$service)) need_reload <- FALSE
}

if (need_reload) {
  # Use full QA dataset to ensure socioeconomic variables are present
  # Default to params$input_gpkg, but check for QA file if using default
  ...
  # Sourced from R/service_config.R (2026-09-01) -- was an independent, unsynced copy that had
  # drifted (fixed 2026-08-31, see docs/pipeline_reference.md B7 for the full incident).
...
# Hardcoded 8-service list (with ratios) replaced 2026-09-02 -- another independent copy found
# alongside the include_services fix above; with drop=FALSE this would otherwise force 3 empty
# columns for the now-untested ratio services. Sourced from R/service_config.R.
...
# Hardcoded 8-service list (with ratios) replaced 2026-09-02 -- see the heatmap chunk above.
svc_order <- service_names()

# Clean up variable names for plotting -- friendly labels matching the paper's own
# tbl-cliffs-delta mapping (paper_draft_5service.qmd), not raw column names
var_label_map <- c(
...
ks_res_plot <- ks_res %>%
  mutate(var_label = dplyr::coalesce(var_label_map[var], gsub("_", " ", var))) %>%
  ...
  ks_clean <- ks_res %>%
    mutate(
      Variable = case_when(
        ...
        grepl("BUILT", var) ~ "Built Area",
        TRUE ~ var
      )
    ) %>%
    select(Service = service, Variable, KS_Stat_D = D, Cliffs_Delta = cliffs_delta, p_adj) %>%
    arrange(desc(abs(Cliffs_Delta)))

  cat("\n\n#### Top 10 Strongest Socioeconomic Associations (Overall)\n\n")
  print(knitr::kable(head(ks_clean, 10), digits = 3))

  cat("\n\n#### Top GDP Associations\n\n")
  print(knitr::kable(ks_clean %>% filter(Variable == "GDP") %>% arrange(desc(abs(Cliffs_Delta))) %>% head(5), digits = 3))

  cat("\n\n#### Top Population Associations\n\n")
  print(knitr::kable(ks_clean %>% filter(Variable == "Population") %>% arrange(desc(abs(Cliffs_Delta))) %>% head(5), digits = 3))
}
```
```
...
if (length(pngs)) {
  rel_dir <- file.path("..", "outputs", "plots", paste0("ks", params$output_suffix))
