...
I had to do this to make sense of the different runs i have done and the data that i need. From individual yearly stats (which ones?) ["raw" data rasters and derived raster algebra products] bi-temporal relative change, key attributes from the input data among others. Need to come out with am ore efficient   
...
#This might not be necessary, the bug was somewhere else  
...
# Reproject both layers to an eqiual area crs (to make sure the we are correctly calculating the areas withing the polygons)
...
# Rasterize the spatial uit polygons
...
# Here i used this clipped layer, because of an error that i thought was due to the extet of bioems being slightly differnet to basins. I clpiied to remove the unmatteches portions but that was not the cause anyway. I don't thin this is necessary,  but i am not repeating this
...
bs <- st_read('/Users/rodriguez/Library/CloudStorage/OneDrive-WorldWildlifeFund,Inc/global_NCP/vector/hydrosheds_lv6_synth-lilling-safeBackup-0001.gpkg')
