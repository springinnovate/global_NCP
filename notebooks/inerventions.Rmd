...
# Extract band names from column names and repeat for each repetition
band_names <- sub(".*_", "", names(all_results)[-ncol(all_results)])
all_results$band <- rep(band_names, n_repetitions)

df <-all_results %>%  filter(band!="nature_access_diff_Sc3v1_PNVnoag-esa2020")
p <- ggplot(all_results, aes(x = band, y = sum)) +
  geom_boxplot() +
  labs(title = "Total estimated Service Gain in units for the target intevention area",
       x = "Band",  # Keep the x-axis label for the overall plot
       y = "Total Sum Value") +
  theme_bw() +
  theme(
    axis.text.x = element_blank(),  # Remove x-axis labels within facets
    axis.ticks.x = element_blank(),  # Remove x-axis ticks within facets
    strip.background = element_blank(),  # Remove facet label background
    strip.text = element_text(face = "bold")  # Make facet labels bold
  ) +
  facet_wrap(~ band, scales = "free_y")  # Create facets with free y-axis scales
# Summarize the results across repetitions for each band
...
all_results <- as_tibble(all_results%>%mutate(band=rownames(all_results))) 
# Summarize the results across repetitions for each band
summarized_results <- all_results %>%
  group_by(band, repetition) %>%  # Group by band only
  summarize(
    overall_mean = mean(mean),
    mean_lower_ci = mean(lower_ci),
    mean_upper_ci = mean(upper_ci),
