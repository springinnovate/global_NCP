...
Additonally: 10km grid on;y on the shoreline for coasstal prtection and facilitate calculation

Rasterized country data (*pyramids*) to speed up/help adding country daa/groupings to the dataset. 

## 2. Data Processing Pipeline

### 2.1  Coastal Protection Aggregation
This data set comes originally as a vector
Included three variables for each year: *convert into math notation*
- Rt: Actual Risk Level)
- Rt_nohab: Potential Risk Level if no ecosystem (expected to remain constant, but necessary to calculate the next ones)
...
Geosharding-based pipeline to extract zonal statistics (mean or sum, depending on the service) from modeled global ES raster layers for two  years (1992-2020) to the 10 km grid (or any poygon vector file).  Values are labeled and appended as new column in an output vector ddataset (gpkg)  
...
Final gpkg with the summary stats for each year for all variables plus sio economic data 
Final gpkg object containing all hotspot-related attributes for each 10 km grid cell.

Map outputs prepardi nqgis (one for the hotspots for each variable )
...
There is an issue with the naming of the variables that has been giving me a lot of trouble 
...
ly <- st_layers(here("data","vector", "grid_10k_coastal.gpkg"))
...
plt_long <- plt %>%
  ...
  filter(!is.na(pct_chg)) %>% filter(!is.na(c_fid)) %>% filter(pct_chg != Inf)  # remove infinite values and thoser whihc Country is NA
...
service_levels <- c("Nature_Access","N_export","N_retention", "N_Ret_Ratio", "Sed_export", "USLE", "Sed_Ret_Ratio", "Rt_nohab","C_Risk",
                    "C_Prot_service", "C_Risk_Red_Ratio", "Pollination") 
...
Add the combos
**Need to retain the original fid (from the whole dataset for eventuqal filtering later, it gets lost somewhere here...)**

```{r get histposts function}
...
## This chunk might not be necessary anymore!!!
...
#| include: false
#| # inpath <- here("output_data")
# hotspots_final <- st_read(paste0(inpath, '/',"10k_grid_hotspots_global.gpkg"))
# hotspots_final$fid <- seq_len(nrow(hotspots_final)) 
plt <- st_drop_geometry(hotspots_sf)
...
# get the columns with the binaries for each individual servic. There must be a better way to o this than by hand
binary_long <- plt %>%
  select(fid, c_fid, all_of(hotspot_cols)) %>%
  pivot_longer(
    cols = -c(fid, c_fid),
    names_to = "service",
...
# Step 3: Pivot long the pct_chg values and normalize service names There MUST be a better way to do this! This is repetitive and easy to get wrong
plt_long <- plt %>%
  pivot_longer(
    cols = matches("pct_chg$"),
    names_to = "service",
  ...
  mutate(service = str_replace(service, "_pct_chg$", "")) %>% #removes "_pct_chg" string, not needed anymore
  mutate(service = str_replace(service, "_mean.*", "")) %>% 
  ...
  filter(!is.na(service)) %>% # this is one of these annpying lines that is too hardcoded inside this thing. it convertsi not a factor but, if some value is not included then the row is dropped. I should set this earlier, not here. i kept it in case i might need it
  filter(!is.na(pct_chg)) %>% filter(pct_chg != Inf) 
  
plt_long <- left_join(plt_long, binary_long, by = c("fid", "c_fid", "service")) 
...
# get a filter with only the services for which i assess hotspots 
service_filter <- unique(plt_long$service)
```

# Next the chart for the whole thing
Not just hotspots.
this is not necessary!!!
```{r scatterplot,fig.height=7, fig.width=7, warning=FALSE}
##Now here for the whole thin
...
plt_long <- plt_long %>% mutate(service = factor(service, levels = service_levels))
plt_long <- plt_long %>% filter(service %in% service_filter)
```
