...
## 1.2 Load The polygons with the NBS areas
```{r}
...
# 2. Crop and mask for the 4 Areas
## 2.1 Test with one raster
This isto process one single raster and thos that require reprojection (access related )
...
Restoration Data is being processed here I designed this process because it hought it would be bettr to oad and process each raster indivdually to avoid memory issues, but now i think i am an idiot because the rasters are not totally loaded anyway. 
...
So far here i have laoded, reprojected when necessary and cropmasked the global ecosystem reasters for each one of the study areas. (NBS) and exported them back to map in Qgis. 
...
# Assuming `rasters_list` is your list of 20 SpatRaster objects, organized by country
# Assuming `vector_list` is your list of 5 SpatVector objects (one for each country)

# Step 1: Split the rasters list into a list of lists, where each sublist contains 4 SpatRasters
baseES <- split(baseES, rep(1:length(ap), each = 4))
