...
latest_dir_boxplots <- file.path("outputs","plots","latest","boxplots")
...
#| results: 'asis'
#| cache: false
box_dir   <- file.path("outputs","plots","latest","boxplots")
...
if (file.exists(lc_gpkg) && exists("plt_long")) {
  ...
  drivers <- list(
    "Forest_Loss" = "ForestLoss_Loss_Forest_2020_1992",
    "Urban_Exp"   = "Expansion_Gain_Urban_2020_1992",
    "Crop_Exp"    = "Expansion_Gain_Cropland_2020_1992"
  )
  ...
  net_col <- "ForestLoss_dir_ch_Forest_2020_1992"
  if (!net_col %in% names(lc_df)) net_col <- NULL

  if (length(drivers) > 0) {
    message("Found granular LC drivers: ", paste(names(drivers), collapse = ", "))
    lc_data_ready <- TRUE
  } else {
    message("No granular driver columns found in LC data.")
  }
} else {
  message("LC data or plt_long missing.")
}
```
...
if (lc_data_ready) {
  ...
  for (d_name in names(drivers)) {
    ...
    driver_out_dir <- file.path(data_dir(), "processed", "hotspots", "drivers")
