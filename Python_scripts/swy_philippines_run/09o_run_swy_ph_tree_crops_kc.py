"""Tree-crop Kc test: 09n (our CN, per-pixel Kc incl. paddy) with Perennial Crop's Kc replaced by
FAO-56 tree-crop coefficients weighted by MapSPAM crop mix (07i_build_kc_tree_crops.py). Everything
else identical to 09n, so 09n -> 09o isolates the tree-crop change. Annual-mean Kc barely changes
(0.990 -> 0.985), but per-pixel values shift by ~0.065 on 24% of the area; this run shows whether
that matters locally.
    ...
    MSYS_NO_PATHCONV=1 docker run --rm -w /scripts_root -e SWY_DATA_ROOT=/scripts_root/data \\
        ...
        /scripts_root/Python_scripts/swy_philippines_run/09o_run_swy_ph_tree_crops_kc.py
"""
import os
...
def main():
    ...
    workspace = os.path.join(DATA_ROOT, "swy/philippines/workspace_becky_inputs_90m_tree_crops_kc")
    ...
    kc_dir = os.path.join(DATA_ROOT, "swy/philippines/inputs/kc_tree_crops_2020")
    kc_path_args = {
    ...
    args = {
        ...
        "results_suffix": "ph_tree_crops_kc",
