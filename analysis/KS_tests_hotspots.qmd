...
```{r ks test}
 load("./data/global_ncp/processed/hotspots_5.RData")
# 
...
files <- save_ecdf_grids(
  hotspots_df, inverse_df,
  vars = vars_to_test,
  ks_res = ks_res,                  # reuse precomputed D
  service_selection = "top_k",
  top_k = 8,                        # show 8 services per variable
  transform_default = "log1p",      # <- force log1p for ALL vars
  transform_by_var = NULL,          # (not needed when forcing)
  out_dir = "output_charts/ecdf",
  prefix = "ecdf",
  width = 10, height = 7, dpi = 300,
  use_ragg = TRUE
