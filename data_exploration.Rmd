...
# split again. We are going to use this a lot
#baseES <- unlist(baseES, recursive=FALSE)
...
baseES <- map2(
  ...
  bkg_wgs,     # The list of backgrounds
  function(rasters, bkg_wgs) {
    # Apply mask to each raster in the sublist using the corresponding country vector
    lapply(rasters, function(raster) merge(raster, bkg_wgs))
  }
)
...
# This extracts the background/the total area of interst. This is necessary becaue we need to know the share of the total area that is under the target 
# class/status (protected area, restoration, land cover wahtever) as a share of the total. This is necessary because we want to compare this share with the share of the service/target data to check xcompare between isnide/outside. This tis the whole purpose of this part 
msk_ap <- baseES_ap[c(1,4,7,10,13)]
...
msk_ap <- map2(msk_ap, bkg_wgs, merge)
...
#####################################################
# leave it here, but thos has the frequencies. from here gettign the shares is very easy. I always foget tho. 

# Set the target CRS to Equal Earth (EPSG:8857) It is absolutely necessary because we are going to calculate shares using landcapemetrics fucntions, not by counting pixels
...
msk_api <- split(msk_api, rep(1:length(bkg), each = 3))3
#Add the background

msk_ap <- map2(
  msk_ap,   # The list of raster sublists
  ,     # The list of country polygons
  function(rasters, vector) {
    # Apply mask to each raster in the sublist using the corresponding country vector
    lapply(rasters, function(raster) mask(raster, vector, inverse=TRUE))
  }
...
freq_all <- lapply(bkg, function(r){
  f <- freq(r)
})

freq_all <- lapply(seq_along(freq_all), function(i) {
  freq_all[[i]]$country <- country_names[i]  # Add country column
  freq_all[[i]]  # Return the modified dataframe
})

freq_all <- do.call(rbind, freq_all)
freq_all <- freq_all %>% mutate(layer = 3)

##################################
################################## This has the count inside outside

freq_ap <- rbind(freq_ap,freq_all)
...
freq_no_ap <- do.call(cbind, freq_no_ap)
...
names(freq_f) <- c("pix_count_pa", "Country","pix_count_area")

freq_f <- freq_f %>% select(c(3,1,2))
names(totSE) <- c("Service", "Country","WholeArea", "PArea")

totSE <- left_join(totSE,freq_f)
totSE <- totSE %>%distinct()
