...
Bars are computed directly from `processed/10k_change_calc.gpkg`. We output two variants—zeros kept vs zeros dropped—both with trimmed tails and a dashed global mean. Pivioted `plt_long` is only necessary for hotspots/KS.
...
```
### Diagnose sign flips (keep0 vs drop0)
...
cmp <- dplyr::full_join(
  signed_keep0 |> dplyr::select(grouping, metric, service, group, mean_keep = mean_chg),
  signed_drop0 |> dplyr::select(grouping, metric, service, group, mean_drop = mean_chg),
  by = c("grouping","metric","service","group")
) |>
  dplyr::mutate(
    sign_keep = sign(mean_keep),
    sign_drop = sign(mean_drop),
    flipped   = sign_keep != sign_drop & !is.na(sign_keep) & !is.na(sign_drop)
  )

# Examples:
# cmp |> dplyr::filter(flipped, grouping == "continent", metric == "pct")
# cmp |> dplyr::filter(service %in% c("C_Risk","C_Risk_Red_Ratio"))
```
