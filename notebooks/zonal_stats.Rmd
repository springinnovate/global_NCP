...
Compute mean for each polygon. Evaluate other metrics
...
# By Continent
set <- 'continent.gpkg' #change to poly_means or just poly everythingh else 
# By Subregion
...
################# For biomes, that are not on the same group
set <- "wwf_biomes.gpkg"
poly <- st_read(here('vector', 'biome_poly_means.gpkg'))

# Set the column to append from
col <- "WWF_biome"
cols <- col
...
## 4. Compute Zonal Statistics with exactextractr for each year
...
Run for: Biomes (done), Continents, income group (done),  wb_region(done), subregion might be more interesting than continent?
