...
- TODO: V2 Simplification. Rely strictly on `orig_fid` generated by the Python summary pipeline to perform fast tabular joins in R. This safely handles dropped/empty cells naturally, completely removing the need for slow spatial coordinate (X/Y) alignments.
...
**Note:** To ensure global comparability, all volumetric services are normalized to a per-hectare basis to correct for pixel area distortion at different latitudes.
