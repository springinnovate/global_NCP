...
| `extract_hotspots.R` | 5-service hotspot extraction (Nature_Access, Pollination, N_retention, Sed_retention, C_Prot_service — retention/protection framing, see `R/service_config.R`) + water/access/combined_cross overlap columns, global scope, pct+abs. Writes `data/processed/hotspots_5service/{pct,abs}/global/`. Renamed 2026-09-01 from `extract_hotspots_5service.R` — the suffix was a leftover from when this was a special variant; it's the canonical script now. | After any change to the service definition or threshold |
| `gdal_rasterize_hotspots_5service.R` | Rasterizes hotspot_count + the 5 per-service flags (from `analysis/hotspot_extraction.qmd`'s output) plus the 3 overlap columns `count_water`/`count_access`/`combined_cross` (from `extract_hotspots.R`'s output) for Rich's beneficiary pipeline, same convention as `gdal_rasterize_hotspots.sh`. | After either source's output changes |
