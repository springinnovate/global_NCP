...
corr_df <- sf::st_read(corr_path, quiet = TRUE) %>%
  ...
  dplyr::select(iso3, nev_name, region_wb, income_grp, continent, region_un, subregion)
...
biome_sf <- sf::st_read(biome_path, quiet = TRUE) %>%
  dplyr::select(WWF_biome) %>% # Select only the biome name and its geometry (sf objects preserve geometry by default)
  sf::st_make_valid()
...
master_grid <- master_grid %>%
  dplyr::select(fid, iso3, nev_name, region_wb, income_grp, continent, region_un, subregion, WWF_biome, geometry)
