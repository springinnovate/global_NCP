...
land_polys <- st_read(
  data_vectors("cartographic_ee_r264_correspondence.gpkg"),
...
# Define grouping variables to attach from the correspondence file.
# NOTE: We are temporarily excluding `WWF_biome` here. The `any_of()` selector
# was silently ignoring it, which is why this chunk was stable.
GROUPING_VARS <- c("nev_name", "region_wb", "income_grp")

# Attach country/regional attributes
message("Attaching country and regional attributes...")
land_attrs <- land_polys |>
  dplyr::select(any_of(GROUPING_VARS))
...
st_write(grid_land, out_path, delete_dsn = TRUE)
