...
message("Bypassing slow spatial join by using `orig_fid` for fast tabular aggregation...")
S_df <- sf::st_drop_geometry(S)
B_df <- sf::st_drop_geometry(B)

if (!"orig_fid" %in% names(S_df) || !"orig_fid" %in% names(B_df)) {
  stop(paste(
    "CRITICAL: `orig_fid` not found in service/beneficiary fragments.",
    "Cannot perform fast tabular join, which is why the script is taking hours.",
    "Please re-run the upstream Python extraction to include `orig_fid`."
  ))
}

# Rename `orig_fid` to `true_fid` to match the existing aggregation logic.
S_df <- S_df %>% dplyr::rename(true_fid = orig_fid) %>% dplyr::filter(!is.na(true_fid))
B_df <- B_df %>% dplyr::rename(true_fid = orig_fid) %>% dplyr::filter(!is.na(true_fid))
message("Re-aggregating fragments into canonical 10km grid cells using `orig_fid`...")
...
sf_f <- master_grid %>%
  dplyr::select(fid) %>%
  dplyr::left_join(joined_tbl, by = "fid")
