...
# Filter Database to specific needs
...
df_1 <- df %>% filter(service != "GDP") %>% (service != "Pop_sqkm")
...
```{r oterh plot, fig.height=9, fig.width=7, warning=FALSE}
library(dplyr)
library(ggplot2)
library(forcats)
...
for (metric in lc_metrics) {
  p <- df_top_bottom %>%
    ...
    scale_color_identity() +
    labs(
      title = paste("Top and Bottom 10% Basins -", metric, "vs. Transformed % Change"),
      x = metric,
      y = "Transformed % Change in ES"
    ) +
    theme_minimal() +
    theme(
      strip.text = element_text(face = "bold"),
