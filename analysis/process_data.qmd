...
# The new, robust workflow reads a clean CSV from the raster-based Python
# pipeline (`summary_pipeline_rasterzones.py`). This completely bypasses all
# vector geometry issues and the complex fragment aggregation logic that was
# previously required when using the old `summary_pipeline_landgrid.py`.
summary_csv_path <- latest_raster_summary("^raster_zonal_summary.*\\.csv$")
summary_df <- read_csv(summary_csv_path, show_col_types = FALSE)

# The master grid already has all attributes. Join by `fid`.
master_path <- file.path(data_dir(), "vector_basedata", "AOOGrid_10x10km_land_4326_python_clean.gpkg")
...
# Perform a simple, non-spatial join by the feature ID. This is much faster
# and more reliable than the previous spatial join operations.
sf_f <- master_grid %>% dplyr::left_join(summary_df, by = "fid")
