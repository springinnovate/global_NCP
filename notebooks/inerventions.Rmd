...
```{r load intevention data}
tiffes <- file.path(here("Interventions", "Brazil_intervention"), list.files(paste0(here("Interventions", "Brazil_intervention")),pattern= 'BRAZIL'))
...
pize# Calculate the number of pixels needed for 30,000 hectares
...
# Create a table grob for the units with smaller font size
unit_table <- tableGrob(unique(all_results[, c("Service", "units")]), 
                        theme = ttheme_minimal(base_size = 8))  # Adjust base_size as needed

# Draw the plot and the unit table using grid
grid.newpage()
pushViewport(viewport(layout = grid.layout(nrow = 1, ncol = 2, widths = c(3, 1))))  # Adjust widths as needed
...
# Draw the unit table in the second column with adjusted position
pushViewport(viewport(layout.pos.col = 2, x = unit(0.8, "npc")))  # Adjust x as needed
grid.draw(unit_table)
popViewport()
...
# Create a table grob for the units
unit_table <- tableGrob(unique(all_results[, c("Service", "units")]), 
                        theme = ttheme_minimal())

# Arrange the plot and the unit table using gridExtra
grid.arrange(plot, unit_table, ncol = 2, widths = c(3, 1))  # Adjust widths as needed
...
## Draw the plot and add unit labels using grid
grid.newpage()
grid.draw(plot)

# Get the viewport for each facet
vp <- viewport(layout = grid.layout(nrow = 3, ncol = 2))  # Adjust nrow/ncol as needed
pushViewport(vp)

# Loop through facets and add unit labels (Corrected)
for (i in seq_along(unit_grobs)) {
  pushViewport(viewport(layout.pos.row = ceiling(i / 2), layout.pos.col = i %% 2 + 1))
  # Add a new viewport for the unit label to the left of the y-axis
  pushViewport(viewport(x = unit(0.0, "npc"), y = unit(0.5, "npc"), width = unit(0.1, "npc"), height = unit(1, "npc"), just = "left"))
  grid.draw(unit_grobs[[i]])
  popViewport()  # Pop the unit label viewport
  popViewport()  # Pop the facet viewport
}
```
