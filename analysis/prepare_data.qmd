...
grid_attrs <- grid_pts %>%
  ...
  # This is a robust way to handle variations in the source data column names.
  {
    df <- .
...
message("Reprojecting to EPSG:4326 before dateline wrap...")
grid_4326 <- st_transform(grid_land_attrs, "EPSG:4326")
...
message("Validating and cleaning geometries post-processing...")
...
if (any(invalid_geoms)) {
  ...
  # Add a zero-buffer to rebuild geometries, which can fix more stubborn issues
  grid_clean <- sf::st_buffer(grid_clean, dist = 0)
}
empty_geoms <- sf::st_is_empty(grid_clean)
if (any(empty_geoms)) {
    message(glue::glue("Found and removed {sum(empty_geoms)} empty geometries."))
    grid_clean <- grid_clean[!empty_geoms, ]
}
...
st_write(grid_clean, out_path, delete_dsn = TRUE)
