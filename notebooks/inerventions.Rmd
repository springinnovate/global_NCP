...
The described workflow is computationally intensive but manageable with modern spatial libraries like terra in R or GDAL in Python.
...
**Note**. Need to think if the normalization is best before or after masking, but i think after will provide slightly more detailed data, but i dont think it changes a lot
...
Also run the normalization nand add them together
...
## 3.4 Sampling and Extracyting values

```{r define vars}
#ok, i  need a template background raster ot sample across it (we could add a restriction that says that it has to be at least x distance from the external border, that sampled points need to be at least x distance between or that the sampling uint is more than one pixel, like clumps of pixels equaling to a target area. that would be fum)
sample <- sample_pixels(base_raster, 30000)
set.seed(123) 
num_iterations <- 30
