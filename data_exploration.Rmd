...
=======
tiffes <- file.path(here(inpath), list.files(here(inpath), pattern='tif'))

n <- 3 #this is therestoration for pollination DO NOT ever do this again
>>>>>>> main
t <- rast(tiffes[[n]])
...
<<<<<<< HEAD
paste0(here(outpath, 'coastal_risk_reduction_'), nam[x], '.tif')
...
#rest naturebase
t <-  list.files(here(inpath), pattern='nbs')
t <- t[2]

rest <- rast(here(inpath, t))

t <- "~/Downloads/nbs_restoration_naturebase.tif.ovr"
...
=======
#This is for the restoration data. 
# keep the conservation also 
tiffes <- file.path(here(inpath), list.files(paste0(here(inpath)),pattern= '2020-1992'))
...
<<<<<<< HEAD
# 4. Extract Summaries Inside/Outside
...
for cosnervation. Once i get this done, i need the same for griscom \##
=======
...
## 4.1 Extract and apply Masks (inside/outside AOI) 

Update the protected areas!!! 
```{r apply mask, eval=TRUE, include=FALSE}
#load protected areas
pa = st_read(here("vector", "areas_nbs2.geojson"))
>>>>>>> main
...
<<<<<<< HEAD
# bkg. <- lapply(bkg, function(r){
#   r <- project(r, equal_earth_crs, res = 30)
#   })
...
pa <- lapply(lc, function(r){
  r <- classify(r, rcl)
})# Load the rasters with the target data.

###### This is not a great way to do this, it is annoying to know where each thing is and go name by name, when they are these very long strings. I have to think how to deal with this better. 
tiffes <- file.path(wd, list.files(wd, pattern="tif"))
#tiffes <-  tiffes[c(36:40, 46:50, 66:70, 86:90)]
tiffes <-  tiffes[c(31:35, 41:45, 81:85)]#, 111:115)]# Conservation. do not incldue Access yet it's thesame crs, but differnetresolution, would have to resample and a lot of other things and that's annoying, it' s easier to do appartand then add the rows. 
>>>>>>> main
...
rm(lc)
<<<<<<< HEAD
# set the projected reference ayatem here, so I on;y have to run this once!
# equal_earth_crs <- "EPSG:8857"
# baseEs <- lapply(baseES, function(r){
#   r <- project(r, equal_earth_crs, method='bilinear')
# })
...
```
################################
## 4.2 Extract values for the density plots
...
##4.3  Extract the shares
#####################################################
```{r extract shares AOI/service prov}
>>>>>>> main
freq_ap <- lapply(msk_ap, function(r){
  f <- freq(r)
})
...
# Set the target CRS to Equal Earth (EPSG:8857) It is necessary because we are going to calculate shares using landscapemetrics functions, not by counting pixels

# equal_earth_crs <- "EPSG:8857"
# baseES_ap <- lapply(baseES_ap, function(r){
#   r <- project(r, equal_earth_crs, method='bilinear')
# })
# baseES_api <- lapply(baseES_api, function(r){
#   r <- project(r, equal_earth_crs, method='bilinear')
# })
# 
# msk_ap <- lapply(msk_ap, function(r){
...
## 4.5 Extract Values and organize the dataftames for charting
