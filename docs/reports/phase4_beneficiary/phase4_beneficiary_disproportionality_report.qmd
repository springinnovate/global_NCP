...
| | |
|---|---|
| **Combined cross-category** | Cells that are simultaneously a water-pathway hotspot (nitrogen *or* sediment export) *and* an access-pathway hotspot (nature access, pollination, *or* coastal risk). The 23.6% figure below is the **reach/beneficiary footprint** — land within 50km downstream or 1hr travel of one of these cells — not the hotspot cells themselves, which cover a much smaller 3.5% of land (see the paper draft abstract). The two are consistent, not contradictory: a buffer zone around a set of cells is always larger than the cells themselves. Both use the same underlying ~4.1B reach-population figure. |
...
**GDP and population** are the strongest signals by a wide margin (δ ≈ 0.48–0.53, the "large effect" band) — beneficiary cells roughly 3–5× higher in per-cell GDP and 2.5–3.5× higher in per-cell population than the rest. These masks already cover a large share of total land area. Buffers are built around where hotspots are, and hotspots concentrate around where people already live and build economic activity.
