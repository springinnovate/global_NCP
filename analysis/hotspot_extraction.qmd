...
# TODO: replace long pivot with per-service aggregation to avoid huge intermediates
# (loop over services, trim, summarise by group; see scratch/make_signed_bars.R)
...
signed_pngs <- list.files(bar_dir, pattern = "^bars_signedline_.*\\.png$", full.names = FALSE)

if (length(signed_pngs)) {
  # Render with paths relative to the HTML output (which lives in analysis/)
  rel_paths <- file.path("..", bar_dir, sort(signed_pngs))
  md <- c(
    "### Signed means (zeros included, trimmed caps)",
