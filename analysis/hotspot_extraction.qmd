...
# Geometry source: prefer the in-memory grid used to build plt_long
sf_base <- if (exists("sf_f") && inherits(sf_f, "sf")) sf_f else sf::st_read(gpkg_grid, quiet = TRUE)

# Stable fid for joining back to geometry
if (!"fid" %in% names(sf_base)) sf_base$fid <- seq_len(nrow(sf_base))
stopifnot(!any(duplicated(sf_base$fid)))
...
run_one_hotset <- function(df, value_col, scope,
  ...
  tibble::tibble(
    ...
    group_val  = ifelse(is.na(group_val), NA_character_, group_val),
    metric     = metric_stub,
    n_hot      = n_hot,
    gpkg       = out_gpkg
  )
...
idx_global_abs <- run_one_hotset(plt_long, value_col = "abs_chg", scope = "global")
idx_global_pct <- run_one_hotset(plt_long, value_col = "pct_chg", scope = "global")
...
for (gc in groupings) {
  ...
  index_rows <- append(index_rows, list(
    purrr::map_dfr(vals, \(v) run_one_hotset(plt_long, "abs_chg", "by_group", gc, v)),
    purrr::map_dfr(vals, \(v) run_one_hotset(plt_long, "pct_chg", "by_group", gc, v))
  ))
}
