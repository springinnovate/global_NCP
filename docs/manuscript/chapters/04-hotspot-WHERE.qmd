...
Direction of change differs by service: negative SPC = declining provision (worse for Pollination, Nature Access); positive SPC = increasing export/risk (worse for N_export, Sed_export, C_Risk).
:::

::: {.callout-note style="font-size: 0.85em;"}
**Updated 2026-09-08**: this used to be split into three parallel tabs per grouping (volumetric/coastal/retention-ratio services) — dropped as part of the B8 pipeline consolidation (`docs/pipeline_reference.md` row B8) once all 5 hotspot-defining services became benefit-framed and each facet already uses `free_y`, so no information was lost by consolidating. The three-way split files (`boxplots_{volumetric,coastal,ratios}_pct.png`) still exist on disk but are orphaned pre-consolidation output — the single `boxplots_pct.png` per grouping below is the current, correct asset.
:::
...
![SPC distribution of hotspot cells (top 5% globally per service) by income group, all 5 hotspot-defining services.](../../../outputs/plots/boxplots_unified/income_grp/boxplots_pct.png){width=100%}

### By World Bank Region

![SPC distribution of hotspot cells by World Bank region. Latin America & Caribbean shows the widest and most intense boxes for Nature Access and Pollination — hotspot severity is both high and highly variable, with some cells approaching complete functional loss (SPC near −200%).](../../../outputs/plots/boxplots_unified/region_wb/boxplots_pct.png){width=100%}

### By WWF Biome

![SPC distribution of hotspot cells by WWF biome.](../../../outputs/plots/boxplots_unified/WWF_biome/boxplots_pct.png){width=100%}
...
# Fixed 2026-09-08: was the pre-redesign 8-service list including 3 ratio columns
# (C_Risk_Red_Ratio, N_Ret_Ratio, Sed_Ret_Ratio) that no longer exist in
# hotspots_global_pct.gpkg as of the 2026-09-03 reversal -- this SELECT would error.
# This qmd doesn't source R/service_config.R, so hardcoded directly (same documented
# exception pattern as paper_draft_5service.qmd's own tbl-cliffs-delta chunk) -- keep
# in sync with that file's SERVICE_AMOUNTS if services ever change.
.svc_cols <- c("C_Risk", "Pollination", "Nature_Access", "Sed_export", "N_export")
...
2. **Lower-middle income countries carry the sharpest per-unit burden.** This income group shows the highest relative hotspot intensity across most services — holding roughly 1.8 times the share of hotspots, relative to its land area, that high-income OECD countries do — making it the single most important socioeconomic risk group in geographic terms.

3. **Biomes diverge sharply.** Mangroves stand out, holding roughly 4.5 times more hotspot area than their global extent would predict, followed distantly by Tropical & Subtropical Moist Broadleaf Forest (roughly 2 times, the next-highest biome). By contrast, boreal forests, deserts, and tundra hold less hotspot area than their extent would predict — suggesting that the speed of tropical ecosystem change far outpaces what biome area alone would predict.
