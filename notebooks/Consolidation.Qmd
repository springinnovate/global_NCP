...
cp2 <- cp2 %>% mutate(fid = dplyr::row_number())
...
combo1 <- c("Rt_service_mean", "Rt_nohab_mean","Pollination","Nature_Access","Sed_export","N_export")
combo2 <- c("Rt_service_mean", "Rt_nohab_mean","Pollination","Nature_Access","Sed_export","N_export") 
  c("N_Ret_Ratio",,"N_retention")
combo3 <- c("Sed_Ret_Ratio",, "USLE")
# ------------------------------------------
...
pct_cutoff <- 0.05  # top/bottom 5%


# add object with the country data:


grid1 <- st_read('/Users/rodriguez/Library/CloudStorage/OneDrive-WorldWildlifeFund,Inc/Ref_Data/vector/grid_10km_f.gpkg')

grid1 <- grid1 %>% 
  st_drop_geometry(grid1) %>% 
  distinct(c_fid, .keep_all = TRUE)
 
# join the additional columns (that's necessary if we want to run the analysis by subregion, specific countries!!!!, just apply syuited filters)

plt_long <- left_join(plt_long, grid1, by= "c_fid")

df <- plt_long
# Optional. Filter by country /other types
#df <- plt_long %>% filter(income_grp == "5. Low income")
...
hotspot_summary <- df_hotspots_negative %>%
  group_by(fid) %>%
  summarise(
  c_fid = first(c_fid),  # bring c_fid along with fid
  hotspot_count = n(),
...
hotspot_cols <- c("Rt_nohab_mean", "Rt_service_mean", "Sed_Ret_Ratio", "Sed_export", "USLE",
                  "N_retention", "Nature_Access", "N_export", "N_Ret_Ratio", "Pollination")
