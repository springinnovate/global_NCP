...
**I Think this is not necessary anymore, these are older versions and i am wasying more time trying to make sense of this than just redoing the whole thing, i just need to find the usle data!!!**

```{r refine columns}
basin <- st_read(set, layer= "hydrosheds_lev6") 
basin <- basin[c(1:13)] # just get the originql columns. Does nothing else
poly <- st_read(set, layer= "layer1") # not necessary aNYMORE, IT UST HAS THE COASTAQP PRTECTION DATA
bas1 <- st_drop_geometry(poly)
bas1 <- bas1[c(1,14,15,17)]
poly <- st_read(set, layer= "1992_2020_lcc_change_metrics") # Not sure here, but will keep for now. 
bas2 <- st_drop_geometry(poly)
bas2 <- bas2[c(1,14:15,23,22,17,18,19,20,24)]
bas2 <- bas2 %>% mutate(HYBAS_ID = as.numeric(HYBAS_ID))
poly <- st_read(set)
