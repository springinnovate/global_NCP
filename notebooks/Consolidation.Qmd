...
plt_long <- plt %>%
  ...
  filter(!is.na(pct_chg)) %>% filter(!is.na(c_fid)) %>% filter(pct_chg != Inf)  # remove infinite values 
...
plt_long <- plt %>%
  ...
  mutate(service = factor(service, levels = c("Nature_Access", "N_Ret_Ratio", "N_export", "Sed_Ret_Ratio", "Sed_export",  "Pollination","C_Risk", "C_Risk_Red_Ratio", "C_Prot_service"))) %>% 
  filter(!is.na(service)) %>% # thisis one of these annpying lines that is too hardcoded inside this thing. it convertsi not a factor but, if some value is not included then the row is dropped. I should set this earlier, not here. i kept it in case i might need it
