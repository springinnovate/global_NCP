...
- `analysis/hotspot_extraction.qmd`: canonical run; builds `plt_long` from `processed/10k_change_calc.gpkg`, extracts hotspots (loss/gain vectors), and exports bar/violin plots under `outputs/plots/latest/{bars|violins}` plus hotspot GPKGs/CSV index.
...
- `analysis/hotspot_sandbox.qmd`: lightweight playground mirroring configs; heavy chunks off by default; use for ad-hoc experiments (e.g., scatters/ECDFs) without touching the main report.
