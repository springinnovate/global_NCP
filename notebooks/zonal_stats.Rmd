...
data_prepped <- zonal_df %>% filter(!is.na(mean)) %>%  # Remove rows with NA mean values
  mutate(region_wb = reorder_within(region_wb, -mean, service))  # Reorder within each service
...
p_2020 <- ggplot() +
  ...
  scale_x_reordered() +
  labs(
    title = "Mean Ecosystem Service Values per WB Region, 2020",
    x = "Country",
    y = "Mean Value"
  ) +
  theme_bw() +
