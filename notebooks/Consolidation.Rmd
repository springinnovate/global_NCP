...
# this is not working now, naming stuctre too complex, adjusting manually for the moment
...
# Define name replacements per object
rename_list <- list(
  # For sf_a[[1]]
  c(
    ...
    n_ret_1992_mean = "N_ret_ratio_1992_mean",
    n_ret_2020_mean = "N_ret_ratio_2020_mean",
    sed_ret_1992_mean = "Sed_ret_ratio_1992_mean",
    sed_ret_2020_mean = "Sed_ret_ratio_2020_mean"
  ),
...
# Apply renaming to each sf object
sf_a <- Map(function(x, renames) {
  dplyr::rename(x, !!!renames)
}, sf_a, rename_list)


sf_a <- lapply(sf_a, function(x) compute_pct_change(x, suffix = c("_sum", "_mean"), drop_columns = TRUE))
