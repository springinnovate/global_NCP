...
3.  **Distribution**: Where are the most multi-service hotspots located?
...
plot_hotness <- function(df, g_var) {
  ...
  d_plot <- d_plot %>% dplyr::mutate(group = factor(group, levels = rev(sort(unique(as.character(group))))))
