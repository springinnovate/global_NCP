...
I still need to generalzie this to wrok with diffenret data inputs/number of differnetservices. There are some poor coding practices that need to be fixed. 
...
```{r density plots 1, echo=FALSE, fig.height=6, fig.width=9}
...
df <- baseES_vf_filtered %>% filter(Country==nam[C]) %>% filter(Service!="Nature Access")
...
t <- ggplot(df, aes(x = Value, fill = set)) +
  ...
  labs(title = paste("Density Plot of Services Inside and Outside Protected Areas -", nam[C]),
       x = "Value",
       y = "Density",
       fill = "Set") +
...
# # This extracts the background/the total area of interest. This is necessary because we need to know the share of the total area that is under the target 
# # class/status (protected area, restoration, land cover whatever) as a share of the total. This is necessary because we want to compare this share with the share of the service/target data to check compare between inside/outside. This is the whole purpose of this part. 
# msk_ap <- baseES_ap[c(1,4,7,10,13)]   # there must be a smarter way to do this. It needs to work with different quantities, not only on a 3 step basis. I know how!
# msk_api <- baseES_api[c(1,4,7,10,13)]
# 
# msk_ap <- lapply(msk_ap, function(r) {
#   # Replace all non-NA values with 1 using ifel()
