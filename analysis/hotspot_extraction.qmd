...
The spatial objects with the necessary  attributes are loaded and reformatted for analysis and chart preparation, that requires to **pivot** the input vector files into the long format, with one row per service/cell combination.
...
svc_order <-  c("C_Risk","N_export","Sed_export","C_Risk_Red_Ratio","N_Ret_Ratio","Sed_Ret_Ratio",
                "Pollination","Nature_Access")
plt_long <- plt_long |>
dplyr::mutate(service = forcats::fct_relevel(service, !!!svc_order, after = Inf))
...
invisible(lapply(names(groupings), function(gc) {
  make_change_bars(
    ...
    stub      = groupings[[gc]],
    svc_order = svc_order,
    svc_order_only = TRUE,   # default; keeps only your listed facets
...
# With global (now no “NA” facet; Global appears at far right)
invisible(lapply(names(groupings), function(gc) {
  make_change_bars(
    group_col = gc,
    stub      = groupings[[gc]],
